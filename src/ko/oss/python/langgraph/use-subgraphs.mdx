...
이 가이드에서는 서브그래프를 사용하는 메커니즘을 설명합니다. 서브그래프는 다른 그래프에서 [노드](/oss/langgraph/graph-api#nodes)로 사용되는 [그래프](/oss/langgraph/graph-api#graphs)입니다.
...
* [그래프를 노드로 추가하기](#add-a-graph-as-a-node) — 서브그래프를 부모의 노드로 직접 추가하며 부모와 **[상태 키](/oss/langgraph/graph-api#state)를 공유**합니다
...
서브그래프를 구현하는 간단한 방법은 다른 그래프의 노드 내부에서 그래프를 호출하는 것입니다. 이 경우 서브그래프는 부모 그래프와 **완전히 다른 스키마**를 가질 수 있습니다(공유 키 없음). 예를 들어, [멀티 에이전트](/oss/langchain/multi-agent) 시스템의 각 에이전트에 대해 비공개 메시지 히스토리를 유지하고 싶을 수 있습니다.
...
부모 그래프와 서브그래프가 [스키마](/oss/langgraph/graph-api#state)의 공유 상태 키(채널)를 통해 통신할 수 있는 경우, 그래프를 다른 그래프의 [노드](/oss/langgraph/graph-api#nodes)로 추가할 수 있습니다. 예를 들어, [멀티 에이전트](/oss/langchain/multi-agent) 시스템에서는 에이전트들이 종종 공유된 [메시지](/oss/langgraph/graph-api#why-use-messages) 키를 통해 통신합니다.
...
서브그래프가 **자체 메모리를 가지도록** 하려면, 적절한 체크포인터 옵션으로 컴파일할 수 있습니다. 이는 [멀티 에이전트](/oss/langchain/multi-agent) 시스템에서 에이전트들이 내부 메시지 히스토리를 추적하고 싶을 때 유용합니다:
...
[영속성](/oss/langgraph/persistence)을 활성화하면, 적절한 메서드를 통해 [그래프 상태를 검사](/oss/langgraph/persistence#checkpoints)(체크포인트)할 수 있습니다. 서브그래프 상태를 보려면 subgraphs 옵션을 사용할 수 있습니다.
