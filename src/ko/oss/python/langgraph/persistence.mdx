...
Thread는 체크포인터가 저장하는 각 체크포인트에 할당된 고유 ID 또는 스레드 식별자입니다. Thread는 일련의 [runs](/langsmith/assistants#execution)의 누적된 상태를 포함합니다. Run이 실행되면, 어시스턴트의 기본 그래프의 [state](/oss/langgraph/graph-api#state)가 스레드에 영속화됩니다.
