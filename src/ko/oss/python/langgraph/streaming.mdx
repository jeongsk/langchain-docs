...
:::python
다음 스트림 모드 중 하나 이상을 리스트로 @[`stream()`][CompiledStateGraph.stream] 또는 @[`astream()`][CompiledStateGraph.astream] 메서드에 전달합니다:
:::

:::js
다음 스트림 모드 중 하나 이상을 리스트로 @[`stream()`][CompiledStateGraph.stream] 메서드에 전달합니다:
:::
...
:::python
LangGraph 그래프는 스트리밍 출력을 이터레이터로 생성하기 위해 @[`.stream()`][Pregel.stream] (동기) 및 @[`.astream()`][Pregel.astream] (비동기) 메서드를 제공합니다.
...
:::js
LangGraph 그래프는 스트리밍 출력을 이터레이터로 생성하기 위해 @[`.stream()`][Pregel.stream] 메서드를 제공합니다.
...
:::python
스트리밍되는 출력에 [서브그래프](/oss/langgraph/use-subgraphs)의 출력을 포함하려면, 부모 그래프의 `.stream()` 메서드에서 `subgraphs=True`로 설정할 수 있습니다. 이렇게 하면 부모 그래프와 모든 서브그래프의 출력이 스트리밍됩니다.
...
1. `LangGraphRunnableConfig`의 `writer` 매개변수를 사용하여 커스텀 데이터를 발생시킵니다.
...
<Tabs>
    ...
    const graph = // ... 이 도구를 사용하는 그래프를 정의합니다
...
:::js
`streamMode: "custom"`을 사용하여 **모든 LLM API**에서 데이터를 스트리밍할 수 있습니다 — 해당 API가 LangChain 채팅 모델 인터페이스를 구현하지 **않더라도** 말입니다.
...
:::js
모델을 초기화할 때 `streaming: false`로 설정합니다.
