...
* [단기 메모리 추가](#add-short-term-memory): 에이전트의 [상태](/oss/langgraph/graph-api#state)의 일부로 단기 메모리를 추가하여 다중 턴 대화를 활성화합니다.
* [장기 메모리 추가](#add-long-term-memory): 세션 간에 사용자별 또는 애플리케이션 수준 데이터를 저장하기 위한 장기 메모리를 추가합니다.

## 단기 메모리 추가

**단기** 메모리(스레드 수준 [지속성](/oss/langgraph/persistence))는 에이전트가 다중 턴 대화를 추적할 수 있게 합니다. 단기 메모리를 추가하려면:
...
그래프에 [서브그래프](/oss/langgraph/use-subgraphs)가 포함된 경우, 부모 그래프를 컴파일할 때만 체크포인터를 제공하면 됩니다. LangGraph는 자동으로 체크포인터를 자식 서브그래프로 전파합니다.
...
서브그래프가 자체 메모리를 가지도록 하려면 적절한 체크포인터 옵션으로 컴파일할 수 있습니다. 이는 [다중 에이전트](/oss/langchain/multi-agent) 시스템에서 에이전트가 내부 메시지 히스토리를 추적하도록 하려는 경우 유용합니다.
...
:::js
메시지 기록을 자르려면 [`trimMessages`](https://js.langchain.com/docs/how_to/trim_messages/) 함수를 사용하세요:
...
:::python
프롬프팅 및 오케스트레이션 로직을 사용하여 메시지 기록을 요약할 수 있습니다. 예를 들어 LangGraph에서 [`MessagesState`](/oss/langgraph/graph-api#working-with-messages-in-graph-state)를 확장하여 `summary` 키를 포함할 수 있습니다:
...
:::js
프롬프팅 및 오케스트레이션 로직을 사용하여 메시지 기록을 요약할 수 있습니다. 예를 들어 LangGraph에서 `messages` 키와 함께 상태에 `summary` 키를 포함할 수 있습니다:
...
그런 다음 기존 요약을 다음 요약의 컨텍스트로 사용하여 채팅 기록의 요약을 생성할 수 있습니다. 이 `summarizeConversation` 노드는 `messages` 상태 키에 일정 수의 메시지가 축적된 후 호출될 수 있습니다.
