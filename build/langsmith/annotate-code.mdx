...
<Check>
LangChain(Python 또는 JS/TS)을 사용하는 경우, 이 섹션을 건너뛰고 [LangChain 전용 안내](/langsmith/trace-with-langchain)로 바로 이동하실 수 있습니다.
</Check>
...
<Note>
`@traceable` 또는 `traceable`을 사용하더라도, 추적을 LangSmith에 로깅하려면 `LANGSMITH_TRACING` 환경 변수를 `'true'`로 설정해야 합니다. 이를 통해 코드를 변경하지 않고도 추적을 켜고 끌 수 있습니다.

또한 API 키를 `LANGSMITH_API_KEY` 환경 변수로 설정해야 합니다(자세한 내용은 [설정](/) 참조).

기본적으로 추적은 `default`라는 이름의 프로젝트에 로깅됩니다. 다른 프로젝트에 추적을 로깅하려면 [이 섹션](/langsmith/log-traces-to-project)을 참조하세요.
</Note>


`@traceable` 데코레이터는 LangSmith Python SDK에서 추적을 로깅하는 간단한 방법입니다. 어떤 함수든 `@traceable`로 데코레이트하기만 하면 됩니다.

동기 함수를 `traceable`로 래핑할 때(예: 아래 예제의 `formatPrompt`), 추적이 올바르게 로깅되도록 호출 시 `await` 키워드를 사용해야 한다는 점에 유의하세요.
...
1. 특정 코드 블록에 대한 추적을 로깅하고 싶을 때.
2. 추적의 입력, 출력 및 기타 속성을 제어하고 싶을 때.
3. 데코레이터나 래퍼를 사용하는 것이 실용적이지 않을 때.
4. 위의 경우 중 일부 또는 전부.
...
LangSmith에 추적을 로깅하는 또 다른 명시적인 방법은 `RunTree` API를 사용하는 것입니다. 이 API를 사용하면 추적을 더 세밀하게 제어할 수 있습니다. 실행과 하위 실행을 수동으로 생성하여 추적을 구성할 수 있습니다. 여전히 `LANGSMITH_API_KEY`를 설정해야 하지만, 이 방법에서는 `LANGSMITH_TRACING`이 필요하지 않습니다.
...
비디오 튜토리얼을 선호하신다면, LangSmith 소개 과정의 [추적 기초 비디오](https://academy.langchain.com/pages/intro-to-langsmith-preview)를 확인하세요.
