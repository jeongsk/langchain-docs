---
title: 로컬에서 평가 실행하기 (Python 전용)
sidebarTitle: 로컬에서 평가 실행하기 (Python)
---

때때로 LangSmith에 결과를 업로드하지 않고 로컬에서 평가를 실행하는 것이 유용할 수 있습니다. 예를 들어, 프롬프트를 빠르게 반복하면서 몇 가지 예제로 간단히 테스트하거나, 대상 함수와 평가자 함수가 올바르게 정의되었는지 검증할 때 이러한 평가를 기록하지 않을 수 있습니다.

LangSmith Python SDK를 사용하고 `evaluate()` / `aevaluate()`에 `upload_results=False`를 전달하면 이를 수행할 수 있습니다.
...
`langsmith>=0.2.0`이 필요합니다. 예제에서는 `pandas`도 사용합니다.
