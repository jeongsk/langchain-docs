...
* 향상된 기능: AutoGen 에이전트에 [지속성](/oss/python/langgraph/persistence), [스트리밍](/langsmith/streaming), [단기 및 장기 메모리](/oss/python/concepts/memory) 등을 추가할 수 있습니다.
* 멀티 에이전트 시스템: 개별 에이전트가 서로 다른 프레임워크로 구축된 [멀티 에이전트 시스템](/oss/python/langchain/multi-agent)을 빌드할 수 있습니다.
...
LangGraph의 [지속성](/oss/python/langgraph/persistence) 기능을 활용하고 있으므로 이제 동일한 스레드 ID를 사용하여 대화를 계속할 수 있습니다. LangGraph는 자동으로 이전 기록을 AutoGen 에이전트에 전달합니다:
