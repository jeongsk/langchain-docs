...
<Note>
LangSmith REST API를 사용할 때는 요청 헤더에 `"x-api-key"`로 API 키를 제공해야 합니다.

API 키가 여러 워크스페이스에 연결된 경우, 헤더에 `"x-tenant-id"`로 사용 중인 워크스페이스를 지정해야 합니다.

간단한 예제에서는 요청 본문에 `dotted_order` 또는 `trace_id` 필드를 설정할 필요가 없습니다. 이 필드들은 시스템에서 자동으로 생성됩니다. 이 방법이 더 간단하지만, LangSmith에서 속도가 느리고 rate limit이 낮습니다.
</Note>
...
더 빠른 실행 수집과 더 높은 rate limit을 위해 POST `/runs/multipart` [link](https://api.smith.langchain.com/redoc#tag/run/operation/multipart_ingest_runs_api_v1_runs_multipart_post) 엔드포인트를 사용할 수 있습니다. 아래는 예제입니다. 실행하려면 `orjson`(빠른 JSON 처리용)과 `requests_toolbelt`가 필요합니다.
