...
레이트 리미터 구성 방법에 대한 자세한 내용은 [langchain](https://python.langchain.com/docs/how_to/chat_model_rate_limiting/) 문서를 참조하세요.
...
`langchain`을 사용하지 않는 경우 `tenacity` (Python) 또는 `backoff` (Python)와 같은 다른 라이브러리를 사용하여 지수 백오프를 사용한 재시도를 구현하거나, 처음부터 직접 구현할 수 있습니다. 이를 수행하는 방법에 대한 몇 가지 예시는 [OpenAI 문서](https://platform.openai.com/docs/guides/rate-limits#retrying-with-exponential-backoff)를 참조하세요.

## max_concurrency 제한하기

애플리케이션과 평가자에 대해 수행하는 동시 호출 수를 제한하는 것은 모델 호출 빈도를 줄이고 이를 통해 속도 제한 오류를 방지하는 또 다른 방법입니다. `max_concurrency`는 [evaluate()](https://docs.smith.langchain.com/reference/python/evaluation/langsmith.evaluation._runner.evaluate) / [aevaluate()](https://docs.smith.langchain.com/reference/python/evaluation/langsmith.evaluation._arunner.aevaluate) 함수에서 직접 설정할 수 있습니다. 이는 데이터셋을 스레드 간에 효과적으로 분할하여 평가를 병렬화합니다.
