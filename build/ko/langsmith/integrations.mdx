---
title: Integrations
sidebarTitle: Overview
mode: wide
---

[LangSmith](https://smith.langchain.com/) provides support for [LangChain](/oss/python/python/langchain/overview) and [LangGraph](/oss/python/python/langgraph/overview) as well as integrations with a growing set of popular [LLM providers](#llm-providers) and [agent frameworks](#agent-frameworks). For setup and usage, refer to the guide pages in the navigation bar.

## Open source frameworks

<Columns cols={2}>
    <Card
    title="LangChain"
    icon="link"
    href="/langsmith/trace-with-langchain" horizontal />

    <Card
    title="LangGraph"
    icon="circle-nodes"
    href="/langsmith/trace-with-langgraph" horizontal />

</Columns>

## LLM providers

<Columns cols={2}>
    <Card
    title="OpenAI"
    icon="/langsmith/images/OpenAI-black-monoblossom.svg"
    href="/langsmith/trace-openai"
    horizontal />

    <Card
    title="Anthropic"
    icon="/langsmith/images/anthropic-icon.svg"
    href="/langsmith/trace-anthropic" horizontal />

    <Card
    title="Google Gemini"
    icon="/langsmith/images/google-gemini-icon.svg" horizontal />

    <Card
    title="Amazon Bedrock"
    icon="/langsmith/images/bedrock-icon.png" horizontal />

    <Card
    title="DeepSeek"
    icon="/langsmith/images/deepseek-icon.svg" horizontal />

    <Card
    title="Mistral"
    icon="/langsmith/images/mistral-icon.svg" horizontal />

</Columns>

## Agent frameworks

<Columns cols={3}>
    <Card
    title="AutoGen"
    icon="/langsmith/images/ag-icon.svg"
    href="/langsmith/trace-with-autogen" horizontal />

    <Card
    title="CrewAI"
    icon="/langsmith/images/crewai-icon.svg"
    href="/langsmith/trace-with-crewai" horizontal />

    <Card
    title="Google ADK"
    icon="/langsmith/images/agent-development-kit.png"
    href="/langsmith/trace-with-google-adk" horizontal />

    <Card
    title="OpenAI Agents"
    icon="/langsmith/images/OpenAI-black-monoblossom.svg"
    href="/langsmith/trace-with-openai-agents-sdk" horizontal />

    <Card
    title="OpenTelemetry"
    icon="/langsmith/images/opentelemetry-icon.svg"
    href="/langsmith/trace-with-opentelemetry" horizontal />

    <Card
    title="Semantic Kernel"
    icon="/langsmith/images/microsoft-icon.svg"
    href="/langsmith/trace-with-semantic-kernel" horizontal />

    <Card
    title="Vercel AI SDK"
    icon="/langsmith/images/vercel-icon-light.svg"
    href="/langsmith/trace-with-vercel-ai-sdk" horizontal />

</Columns>

## Other

<Columns cols={2}>
    <Card
    title="Instructor"
    icon="/langsmith/images/instructor-icon.svg"
    href="/langsmith/trace-with-instructor" horizontal />

    <Card
    title="Claude Code"
    icon="/langsmith/images/claude.svg"
    href="/langsmith/trace-claude-code" horizontal />

</Columns>

---

<Callout icon="pen-to-square" iconType="regular">
  [Edit the source of this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/ko/langsmith/integrations.mdx)
</Callout>
