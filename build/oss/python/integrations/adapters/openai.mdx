...
많은 사람들이 OpenAI로 시작하지만 다른 모델을 탐색하고 싶어합니다. LangChain은 다양한 모델 공급자와 통합되어 있어 이를 쉽게 수행할 수 있습니다. LangChain은 자체 메시지 및 모델 API를 가지고 있지만, LangChain 모델을 OpenAI API에 적응시키는 어댑터를 제공하여 다른 모델을 최대한 쉽게 탐색할 수 있도록 했습니다.
