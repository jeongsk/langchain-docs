...
>[Etherscan](https://docs.etherscan.io/)은 Ethereum, 즉 분산형 스마트 계약 플랫폼을 위한 선도적인 블록체인 탐색기, 검색, API 및 분석 플랫폼입니다.

## 개요

`Etherscan` 로더는 `Etherscan API`를 사용하여 `Ethereum Mainnet`의 특정 계정에 대한 거래 내역을 로드합니다.

계속하려면 `Etherscan api key`가 필요합니다. 무료 API 키는 초당 5회 호출 할당량을 가집니다.

로더는 다음 6가지 기능을 지원합니다:

* Ethereum Mainet에서 특정 계정의 일반 거래 검색
* Ethereum Mainet에서 특정 계정의 내부 거래 검색
* Ethereum Mainet에서 특정 계정의 erc20 거래 검색
* Ethereum Mainet에서 특정 계정의 erc721 거래 검색
* Ethereum Mainet에서 특정 계정의 erc1155 거래 검색
* Ethereum Mainet에서 특정 계정의 wei 단위 이더리움 잔액 검색

계정에 해당 거래가 없는 경우 로더는 하나의 문서가 포함된 목록을 반환합니다. 문서의 내용은 ''입니다.

위에서 언급한 다양한 기능에 액세스하기 위해 로더에 다른 필터를 전달할 수 있습니다:
...
질문이 있으시면 [Etherscan API Doc](https://etherscan.io/tx/0x0ffa32c787b1398f44303f731cb06678e086e4f82ce07cebf75e99bb7c079c77)에 액세스하거나 [i@inevitable.tech](mailto:i@inevitable.tech)로 문의하세요.

거래 내역과 관련된 모든 함수는 Etherscan 제한으로 인해 최대 1000개의 내역으로 제한됩니다. 필요한 거래 내역을 찾기 위해 다음 매개변수를 사용할 수 있습니다:

* offset: 기본값은 20입니다. 한 번에 20개의 거래를 표시합니다
* page: 기본값은 1입니다. 페이지네이션을 제어합니다.
* start_block: 기본값은 0입니다. 거래 내역은 0 블록부터 시작합니다.
* end_block: 기본값은 99999999입니다. 거래 내역은 99999999 블록에서 시작합니다
* sort: "desc" 또는 "asc". 최신 거래를 얻으려면 기본값으로 "desc"로 설정합니다.
