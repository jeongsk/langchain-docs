...
Unstructured를 로컬로 설정하는 방법에 대한 자세한 지침은 [이 가이드](/oss/python/integrations/providers/unstructured/)를 참조하세요. 여기에는 필요한 시스템 종속성 설정이 포함됩니다.
...
내부적으로 Unstructured는 텍스트의 다른 청크에 대해 다른 "요소"를 생성합니다. 기본적으로 이들을 결합하지만, `mode="elements"`를 지정하여 이러한 분리를 쉽게 유지할 수 있습니다.
...
생성자에서 `process_attachments=True`를 설정하여 `UnstructuredEmailLoader`로 첨부 파일을 처리할 수 있습니다. 기본적으로 첨부 파일은 `unstructured`의 `partition` 함수를 사용하여 파티션됩니다. `attachment_partitioner` kwarg에 함수를 전달하여 다른 파티셔닝 함수를 사용할 수 있습니다.
