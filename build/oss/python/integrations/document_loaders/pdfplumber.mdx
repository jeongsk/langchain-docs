...
PyMuPDF와 마찬가지로 출력 Document는 PDF 및 페이지에 대한 상세한 메타데이터를 포함하며, 페이지당 하나의 문서를 반환합니다.
