...
>[TOML](https://en.wikipedia.org/wiki/TOML)은 구성 파일용 파일 형식입니다. 읽고 쓰기 쉽도록 설계되었으며 딕셔너리에 명확하게 매핑되도록 설계되었습니다. 사양은 오픈 소스입니다. `TOML`은 많은 프로그래밍 언어로 구현되어 있습니다. `TOML`이라는 이름은 창시자인 Tom Preston-Werner를 언급하여 "Tom's Obvious, Minimal Language"의 약어입니다.
