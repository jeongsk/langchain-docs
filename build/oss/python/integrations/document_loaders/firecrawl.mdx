...
[FireCrawl](https://firecrawl.dev/?ref=langchain)은 모든 웹사이트를 크롤링하고 LLM에 최적화된 데이터로 변환합니다. 모든 접근 가능한 하위 페이지를 크롤링하고 각 페이지에 대한 깔끔한 markdown과 메타데이터를 제공합니다. 사이트맵이 필요하지 않습니다.

FireCrawl은 역방향 프록시, 캐싱, 속도 제한, JavaScript로 차단된 콘텐츠와 같은 복잡한 작업을 처리합니다. [mendable.ai](https://mendable.ai) 팀에서 제작했습니다.
...
자신의 API 키를 받아야 합니다. [firecrawl.dev](https://firecrawl.dev)를 참조하세요
...
로더에 `params`를 전달할 수도 있습니다. 이것은 크롤러에 전달할 옵션 사전입니다. 자세한 내용은 [FireCrawl API 문서](https://github.com/mendableai/firecrawl-py)를 참조하세요.
...
로더에 `params`를 전달할 수도 있습니다. 이것은 로더에 전달할 옵션 사전입니다. 자세한 내용은 [FireCrawl API 문서](https://github.com/mendableai/firecrawl-py)를 참조하세요.

## API reference

모든 `FireCrawlLoader` 기능 및 구성에 대한 자세한 문서는 API 참조를 확인하세요: [python.langchain.com/api_reference/community/document_loaders/langchain_community.document_loaders.firecrawl.FireCrawlLoader.html](https://python.langchain.com/api_reference/community/document_loaders/langchain_community.document_loaders.firecrawl.FireCrawlLoader.html)
