...
이 가이드는 `PDFMiner` [문서 로더](https://python.langchain.com/docs/concepts/document_loaders) 시작하기에 대한 간략한 개요를 제공합니다. 모든 __ModuleName__Loader 기능 및 구성에 대한 자세한 문서는 [API 참조](https://python.langchain.com/api_reference/community/document_loaders/langchain_community.document_loaders.pdf.PDFMinerLoader.html)를 참조하세요.
...
모델 호출의 최고 수준 자동 추적을 원하면 아래 주석을 제거하여 [LangSmith](https://docs.smith.langchain.com/) API 키를 설정할 수도 있습니다:
...
**langchain-community** 및 **pdfminer**를 설치합니다.
...
## 분할 모드 및 사용자 정의 페이지 구분 기호

PDF 파일을 로드할 때 두 가지 방식으로 분할할 수 있습니다:
...
### 페이지별로 PDF 추출. 각 페이지는 langchain Document 객체로 추출됩니다
...
이 모드에서는 PDF가 페이지별로 분할되며 결과 Documents 메타데이터에 페이지 번호가 포함됩니다. 그러나 일부 경우에는 PDF를 단일 텍스트 흐름으로 처리하고자 할 수 있습니다(단락을 절반으로 자르지 않도록). 이 경우 *single* 모드를 사용할 수 있습니다:

### 전체 PDF를 단일 langchain Document 객체로 추출
...
논리적으로 이 모드에서는 'page_number' 메타데이터가 사라집니다. 텍스트 흐름에서 페이지가 끝나는 위치를 명확하게 식별하는 방법은 다음과 같습니다:

### *single* 모드에서 페이지 끝을 식별하기 위해 사용자 정의 *pages_delimiter* 추가
