...
`langchain-box` 패키지는 Box에서 파일을 인덱싱하는 두 가지 방법인 `BoxLoader`와 `BoxBlobLoader`를 제공합니다. `BoxLoader`를 사용하면 Box에서 텍스트 표현이 있는 파일의 텍스트 표현을 수집할 수 있습니다. `BoxBlobLoader`를 사용하면 선택한 Blob 파서로 처리하기 위해 모든 문서 또는 이미지 파일의 Blob을 다운로드할 수 있습니다.

이 노트북은 두 가지 방법을 시작하는 방법을 자세히 설명합니다. 모든 BoxLoader 기능 및 구성에 대한 자세한 문서는 [BoxLoader](https://python.langchain.com/api_reference/box/document_loaders/langchain_box.document_loaders.box.BoxLoader.html) 및 [BoxBlobLoader](https://python.langchain.com/api_reference/box/document_loaders/langchain_box.blob_loaders.box.BoxBlobLoader.html)의 API Reference 페이지를 참조하세요.

## 개요

`BoxLoader` 클래스는 Box에서 비정형 콘텐츠를 LangChain의 [`Document`](https://reference.langchain.com/python/langchain_core/documents/#langchain_core.documents.base.Document) 형식으로 가져오는 데 도움이 됩니다. Box 파일 ID가 포함된 `List[str]` 또는 Box 폴더 ID가 포함된 `str`을 사용하여 이 작업을 수행할 수 있습니다.

`BoxBlobLoader` 클래스는 Box에서 비정형 콘텐츠를 LangChain의 `Blob` 형식으로 가져오는 데 도움이 됩니다. Box 파일 ID가 포함된 `List[str]`, Box 폴더 ID가 포함된 `str`, 검색 쿼리 또는 `BoxMetadataQuery`를 사용하여 이 작업을 수행할 수 있습니다.

폴더 ID로 폴더에서 파일을 가져오는 경우 `Bool`을 설정하여 로더가 해당 폴더의 모든 하위 폴더도 가져오도록 지시할 수 있습니다.

<Info>
Box 인스턴스에는 페타바이트의 파일이 포함될 수 있으며 폴더에는 수백만 개의 파일이 포함될 수 있습니다. 인덱싱할 폴더를 선택할 때는 의도적으로 선택하세요. 그리고 폴더 0에서 재귀적으로 모든 파일을 가져오지 않는 것이 좋습니다. 폴더 ID 0은 루트 폴더입니다.
</Info>
...
* [Box 앱](https://developer.box.com/guides/getting-started/first-application/) — [개발자 콘솔](https://account.box.com/developers/console)에서 구성되며 Box AI의 경우 `Manage AI` 범위를 활성화해야 합니다. 여기에서 인증 방법도 선택합니다.
...
이 예제에서는 [토큰 인증](https://developer.box.com/guides/authentication/tokens/developer-tokens)을 사용합니다. 이는 모든 [인증 방법](https://developer.box.com/guides/authentication/)과 함께 사용할 수 있습니다. 어떤 방법으로든 토큰을 가져오기만 하면 됩니다. `langchain-box`에서 다른 인증 유형을 사용하는 방법에 대해 자세히 알아보려면 [Box 프로바이더](/oss/python/integrations/providers/box) 문서를 참조하세요.
...
파일을 로드하려면 인스턴스화 시 파일 ID의 `List`를 제공해야 합니다.

필요한 정보는 1가지입니다:

* **box_file_ids** (`List[str]`)- Box 파일 ID 목록입니다.
...
폴더에서 파일을 로드하려면 인스턴스화 시 Box 폴더 ID가 포함된 `str`을 제공해야 합니다.

필요한 정보는 1가지입니다:

* **box_folder_id** (`str`)- Box 폴더 ID가 포함된 문자열입니다.
...
파일을 검색해야 하는 경우 `BoxBlobLoader`는 두 가지 방법을 제공합니다. 먼저 검색 범위를 좁히기 위한 선택적 검색 옵션과 함께 전체 텍스트 검색을 수행할 수 있습니다.

필요한 정보는 1가지입니다:

* **query** (`str`)- 수행할 검색 쿼리가 포함된 문자열입니다.
...
Box Metadata를 기반으로 콘텐츠를 검색할 수도 있습니다. Box 인스턴스에서 Metadata를 사용하는 경우 특정 Metadata Template이 첨부되어 있고 지난 분기에 생성된 총액이 $500 이상인 송장을 반환하는 것과 같이 특정 기준을 충족하는 모든 문서를 검색할 수 있습니다.
...
모든 Box 커넥터는 Box `FileFull` 객체에서 추가 필드를 선택하여 사용자 정의 LangChain 메타데이터로 반환하는 기능을 제공합니다. 각 객체는 `extra_fields=["shared_link"]`와 같이 반환 객체의 json 키가 포함된 선택적 `List[str]`인 `extra_fields`를 허용합니다.

커넥터는 이 필드를 통합이 작동하는 데 필요한 필드 목록에 추가한 다음 결과를 `"metadata" : { "source" : "source, "shared_link" : "shared_link" }`와 같이 [`Document`](https://reference.langchain.com/python/langchain_core/documents/#langchain_core.documents.base.Document) 또는 `Blob`에 반환된 메타데이터에 추가합니다. 해당 파일에서 필드를 사용할 수 없는 경우 `"shared_link" : ""`와 같이 빈 문자열로 반환됩니다.
...
질문이 있는 경우 [개발자 문서](https://developer.box.com)를 확인하거나 [개발자 커뮤니티](https://community.box.com)에서 문의할 수 있습니다.
