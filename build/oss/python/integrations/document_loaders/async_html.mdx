...
# 웹 요청을 위해 프록시를 사용해야 하는 경우(예: http_proxy/https_proxy 환경 변수 사용),
# 다음과 같이 trust_env=True를 명시적으로 설정하세요:
# loader = AsyncHtmlLoader(urls, trust_env=True)
# 그렇지 않으면 aiohttp 세션이 기본적으로 프록시를 인식하지 못하기 때문에 loader.load()가 멈출 수 있습니다
docs = loader.load()
```
