...
이 가이드는 Cerebras [채팅 모델](/oss/python/langchain/models) 시작하기를 위한 빠른 개요를 제공합니다. 모든 ChatCerebras 기능 및 구성에 대한 자세한 문서는 [API 레퍼런스](https://python.langchain.com/api_reference/cerebras/chat_models/langchain_cerebras.chat_models.ChatCerebras.html#)를 참조하세요.

Cerebras에서는 세계 최대이자 가장 빠른 AI 프로세서인 Wafer-Scale Engine-3(WSE-3)를 개발했습니다. WSE-3로 구동되는 Cerebras CS-3 시스템은 비할 데 없는 성능과 확장성으로 생성형 AI 훈련 및 추론의 표준을 설정하는 새로운 클래스의 AI 슈퍼컴퓨터를 나타냅니다.
...
CS-3 시스템은 신속하고 쉽게 클러스터링하여 세계 최대의 AI 슈퍼컴퓨터를 생성할 수 있으며, 최대 모델을 배치하고 실행하기 쉽게 만듭니다. 주요 기업, 연구 기관 및 정부 기관에서 이미 Cerebras 솔루션을 사용하여 독점 모델을 개발하고 인기 있는 오픈 소스 모델을 훈련하고 있습니다.

Cerebras의 성능을 경험하고 싶으신가요? 더 많은 리소스를 확인하고 Cerebras Cloud 또는 온프레미스 배포를 통해 우리 기술에 액세스하는 옵션을 탐색하려면 [웹사이트](https://cerebras.ai)를 확인하세요!
...
[cloud.cerebras.ai](https://cloud.cerebras.ai/)에서 API 키를 받아 환경 변수에 추가하세요:
...
LangChain Cerebras 통합은 `langchain-cerebras` 패키지에 있습니다:
...
모든 ChatCerebras 기능 및 구성에 대한 자세한 문서는 API 레퍼런스를 참조하세요: [python.langchain.com/api_reference/cerebras/chat_models/langchain_cerebras.chat_models.ChatCerebras.html#](https://python.langchain.com/api_reference/cerebras/chat_models/langchain_cerebras.chat_models.ChatCerebras.html#)
