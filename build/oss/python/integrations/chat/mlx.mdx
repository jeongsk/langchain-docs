...
이 노트북은 `MLX` LLM을 채팅 모델로 시작하는 방법을 보여줍니다.
...
2. `ChatMLX` 클래스를 활용하여 이러한 LLM 중 하나가 LangChain의 [Chat Messages](https://python.langchain.com/docs/modules/model_io/chat/#messages) 추상화와 인터페이스할 수 있도록 합니다.
3. 오픈 소스 LLM을 사용하여 `ChatAgent` 파이프라인을 구동하는 방법을 보여줍니다.
...
LLM 호출을 위해 채팅 메시지가 어떻게 형식화되는지 검사합니다.
...
> 참고: 이 섹션을 실행하려면 [SerpAPI Token](https://serpapi.com/)이 환경 변수로 저장되어 있어야 합니다: `SERPAPI_API_KEY`
...
`react-json` 스타일 프롬프트와 검색 엔진 및 계산기에 대한 액세스 권한으로 에이전트를 구성합니다.
