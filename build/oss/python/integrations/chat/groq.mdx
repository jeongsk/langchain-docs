...
이 문서는 Groq [chat models](/oss/python/langchain/models)을 시작하는 데 도움을 드립니다. 모든 ChatGroq 기능 및 구성에 대한 자세한 문서는 [API reference](https://python.langchain.com/api_reference/groq/chat_models/langchain_groq.chat_models.ChatGroq.html)를 참조하세요. 모든 Groq 모델 목록은 이 [링크](https://console.groq.com/docs/models?utm_source=langchain)를 방문하세요.
...
Groq 모델에 액세스하려면 Groq 계정을 만들고 API 키를 받은 다음 `langchain-groq` 통합 패키지를 설치해야 합니다.

### 자격 증명

[Groq console](https://console.groq.com/login?utm_source=langchain&utm_content=chat_page)로 이동하여 Groq에 가입하고 API 키를 생성하세요. 완료되면 GROQ_API_KEY 환경 변수를 설정하세요:
...
LangChain Groq 통합은 `langchain-groq` 패키지에 포함되어 있습니다:
...
`reasoning_format`을 설정하기로 선택한 경우, 사용 중인 모델이 이를 지원하는지 확인해야 합니다. 지원되는 모델 목록은 [Groq documentation](https://console.groq.com/docs/reasoning)에서 확인할 수 있습니다.
...
모든 ChatGroq 기능 및 구성에 대한 자세한 문서는 [API reference](https://python.langchain.com/api_reference/groq/chat_models/langchain_groq.chat_models.ChatGroq.html)를 참조하세요.
