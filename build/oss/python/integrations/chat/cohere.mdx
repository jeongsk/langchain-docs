...
이 노트북은 [Cohere 채팅 모델](https://cohere.com/chat) 시작하기를 다룹니다.

모든 속성 및 메서드에 대한 자세한 문서는 [API 레퍼런스](https://python.langchain.com/api_reference/community/chat_models/langchain_community.chat_models.cohere.ChatCohere.html)를 참조하세요.

## 설정

통합은 `langchain-cohere` 패키지에 있습니다. 다음과 같이 설치할 수 있습니다:

```bash
pip install -U langchain-cohere
```

[Cohere API 키](https://cohere.com/)를 받아 `COHERE_API_KEY` 환경 변수를 설정해야 합니다:
...
최고 수준의 관찰 기능을 위해 [LangSmith](https://smith.langchain.com/)를 설정하는 것도 도움이 됩니다(필수는 아님):
