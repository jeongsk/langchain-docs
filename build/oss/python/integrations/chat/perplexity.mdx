...
이 페이지는 Perplexity [chat models](/oss/python/langchain/models/)를 시작하는 데 도움을 드립니다. 모든 `ChatPerplexity` 기능 및 구성에 대한 자세한 문서는 [API reference](https://python.langchain.com/api_reference/perplexity/chat_models/langchain_perplexity.chat_models.ChatPerplexity.html)를 참조하세요.
...
Perplexity 모델에 접근하려면 Perplexity 계정을 만들고 API 키를 받은 후 `langchain-perplexity` 통합 패키지를 설치해야 합니다.

### Credentials

[이 페이지](https://www.perplexity.ai/)로 이동하여 Perplexity에 가입하고 API 키를 생성하세요. 완료되면 `PPLX_API_KEY` 환경 변수를 설정하세요:
...
모델 호출의 자동 추적을 활성화하려면 [LangSmith](https://docs.smith.langchain.com/) API 키를 설정할 수 있습니다:
...
사용 가능한 모델 목록은 [여기](https://docs.perplexity.ai/docs/model-cards)에서 확인할 수 있습니다. 재현성을 위해 이 노트북에서는 API 키를 입력으로 받아 동적으로 설정할 수 있습니다.
...
프롬프트를 일반적으로 형식화하고 구조화할 수 있습니다. 다음 예제에서는 모델에게 고양이에 대한 농담을 해달라고 요청합니다.
...
`ChatPerplexity` 클래스를 통해 Perplexity 전용 매개변수를 사용할 수도 있습니다. 예를 들어, 아래와 같이 extra_body 매개변수를 사용하여 search_domain_filter, return_images, return_related_questions 또는 search_recency_filter와 같은 매개변수를 사용할 수 있습니다:
...
특별한 매개변수를 전달할 필요가 없습니다. 목록은 `response.additional_kwargs["search_results"]`에 위치합니다.
