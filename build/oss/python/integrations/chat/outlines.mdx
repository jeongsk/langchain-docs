...
이 문서는 Outlines [chat models](/oss/python/langchain/models/)를 시작하는 데 도움을 드립니다. 모든 ChatOutlines 기능 및 구성에 대한 자세한 문서는 [API reference](https://python.langchain.com/api_reference/community/chat_models/langchain_community.chat_models.outlines.ChatOutlines.html)를 참조하세요.

[Outlines](https://github.com/outlines-dev/outlines)는 제약적 언어 생성을 위한 라이브러리입니다. 다양한 백엔드를 사용하여 대규모 언어 모델(LLM)을 사용하면서 생성된 출력에 제약 조건을 적용할 수 있습니다.
...
Outlines 모델에 접근하려면 huggingface에서 모델 가중치를 다운로드할 수 있는 인터넷 연결이 필요합니다. 사용하는 백엔드에 따라 필요한 의존성을 설치해야 합니다 ([Outlines docs](https://dottxt-ai.github.io/outlines/latest/installation/) 참조).

### Credentials

Outlines에는 내장 인증 메커니즘이 없습니다.

### Installation

LangChain Outlines 통합은 `langchain-community` 패키지에 포함되어 있으며 `outlines` 라이브러리가 필요합니다:
...
ChatOutlines와 함께 LangChain의 Structured Output을 사용할 수도 있습니다:
...
모든 ChatOutlines 기능 및 구성에 대한 자세한 문서는 API reference를 참조하세요: [python.langchain.com/api_reference/community/chat_models/langchain_community.chat_models.outlines.ChatOutlines.html](https://python.langchain.com/api_reference/community/chat_models/langchain_community.chat_models.outlines.ChatOutlines.html)
