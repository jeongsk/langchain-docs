...
[GPTRouter](https://github.com/Writesonic/GPTRouter)는 30개 이상의 LLM, 비전 및 이미지 모델에 대한 범용 API를 제공하는 오픈 소스 LLM API Gateway로, 가동 시간 및 지연 시간을 기반으로 한 스마트 대체, 자동 재시도 및 스트리밍을 지원합니다.

이 노트북은 LangChain과 GPTRouter I/O 라이브러리를 시작하는 방법을 다룹니다.

* `GPT_ROUTER_API_KEY` 환경 변수 설정
* 또는 `gpt_router_api_key` 키워드 인수 사용
