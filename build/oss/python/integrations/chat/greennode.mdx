...
>[GreenNode](https://greennode.ai/)는 글로벌 AI 솔루션 제공업체이자 **NVIDIA Preferred Partner**로서, 미국, MENA, APAC 지역의 기업들에게 인프라에서 애플리케이션에 이르는 풀스택 AI 역량을 제공합니다. **세계적 수준의 인프라**(LEED Gold, TIA‑942, Uptime Tier III)에서 운영되는 GreenNode는 기업, 스타트업, 연구자들에게 포괄적인 AI 서비스 제품군을 제공합니다.

이 페이지는 GreenNode Serverless AI [chat models](/oss/python/langchain/models)을 시작하는 데 도움을 드립니다. 모든 ChatGreenNode 기능 및 구성에 대한 자세한 문서는 [API reference](https://python.langchain.com/api_reference/greennode/chat_models/langchain_greennode.chat_models.ChatGreenNode.html)를 참조하세요.
...
GreenNode 모델에 액세스하려면 GreenNode 계정을 만들고 API 키를 받은 다음 `langchain-greennode` 통합 패키지를 설치해야 합니다.

### 자격 증명

[이 페이지](https://aiplatform.console.greennode.ai/api-keys)로 이동하여 GreenNode AI Platform에 가입하고 API 키를 생성하세요. 완료되면 GREENNODE_API_KEY 환경 변수를 설정하세요:
...
LangChain GreenNode 통합은 `langchain-greennode` 패키지에 포함되어 있습니다:
...
LangChain 체인과 에이전트에서 `ChatGreenNode`를 사용할 수 있습니다:
...
GreenNode Serverless AI API에 대한 자세한 내용은 [GreenNode Serverless AI Documentation](https://helpdesk.greennode.ai/portal/en/kb/articles/greennode-maas-api)을 방문하세요.
