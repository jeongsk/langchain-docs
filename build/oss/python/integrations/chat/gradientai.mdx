...
이 문서는 DigitalOcean Gradient Chat Models을 시작하는 데 도움을 드립니다.
...
DigitalOcean에 계정을 생성하고, Gradient Platform에서 `DIGITALOCEAN_INFERENCE_KEY` API 키를 획득한 후, `langchain-gradient` 통합 패키지를 설치하세요.
...
3. Create model access key를 클릭하고, 이름을 입력한 후 키를 생성합니다.

완료되면 `DIGITALOCEAN_INFERENCE_KEY` 환경 변수를 설정하세요:
...
DigitalOcean Gradient 통합은 `langchain-gradient` 패키지에 포함되어 있습니다:
...
다음과 같이 모델을 프롬프트 템플릿과 연결할 수 있습니다:
...
모든 ChatGradient 기능 및 구성에 대한 자세한 문서는 API 참조를 확인하세요.
