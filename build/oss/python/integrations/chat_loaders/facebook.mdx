...
이 노트북은 Facebook의 데이터를 파인튜닝할 수 있는 형식으로 로드하는 방법을 보여줍니다. 전체 단계는 다음과 같습니다:

1. 메신저 데이터를 디스크에 다운로드합니다.
2. Chat Loader를 생성하고 `loader.load()` (또는 `loader.lazy_load()`)를 호출하여 변환을 수행합니다.
3. 선택적으로 `merge_chat_runs`를 사용하여 동일한 발신자의 연속된 메시지를 결합하거나, `map_ai_messages`를 사용하여 지정된 발신자의 메시지를 "AIMessage" 클래스로 변환할 수 있습니다. 이 작업을 완료한 후 `convert_messages_for_finetuning`을 호출하여 파인튜닝용 데이터를 준비합니다.

이 작업이 완료되면 모델을 파인튜닝할 수 있습니다. 이를 위해 다음 단계를 완료합니다:

4. OpenAI에 메시지를 업로드하고 파인튜닝 작업을 실행합니다.
6. LangChain 앱에서 결과 모델을 사용합니다!
...
이 가이드에서 사용할 예제 덤프를 [이 Google Drive 링크](https://drive.google.com/file/d/1rh1s1o2i7B-Sk1v9o8KNgivLVGwJ-osV/view?usp=sharing)에서 호스팅하고 있습니다.
...
전체 채팅 디렉토리용과 개별 파일 로드용으로 2개의 서로 다른 `FacebookMessengerChatLoader` 클래스가 있습니다.
...
`load()`를 호출하면 추출할 수 있는 모든 채팅 메시지가 human 메시지로 반환됩니다. 챗봇과 대화할 때 대화는 일반적으로 실제 대화에 비해 더 엄격한 교대 대화 패턴을 따릅니다.

메시지 "실행"(동일한 발신자의 연속 메시지)을 병합하고 "AI"를 나타낼 발신자를 선택할 수 있습니다. 파인튜닝된 LLM은 이러한 AI 메시지를 생성하도록 학습합니다.
...
OpenAI는 현재 파인튜닝 작업에 최소 10개의 학습 예제를 요구하지만, 대부분의 작업에는 50-100개를 권장합니다. 채팅 세션이 9개뿐이므로 각 학습 예제가 전체 대화의 일부로 구성되도록 (선택적으로 일부 겹침과 함께) 세분화할 수 있습니다.

Facebook 채팅 세션(1인당 1개)은 종종 여러 날과 대화에 걸쳐 있으므로 어쨌든 장거리 종속성이 모델링하는 데 그다지 중요하지 않을 수 있습니다.
...
이제 모델을 파인튜닝할 시간입니다. `openai`가 설치되어 있고 `OPENAI_API_KEY`가 적절하게 설정되어 있는지 확인하세요.
...
파일이 준비되었으므로 이제 학습 작업을 시작할 차례입니다.
...
모델이 준비되는 동안 차 한 잔 드세요. 시간이 좀 걸릴 수 있습니다!
