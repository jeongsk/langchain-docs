...
LangChain은 벡터 스토어를 위한 통합된 인터페이스를 제공하여 다음 작업을 수행할 수 있습니다:

- `add_documents` - 스토어에 문서를 추가합니다.
- `delete` - ID로 저장된 문서를 제거합니다.
- `similarity_search` - 의미적으로 유사한 문서를 쿼리합니다.

이러한 추상화를 통해 애플리케이션 로직을 변경하지 않고도 서로 다른 구현 간에 전환할 수 있습니다.
...
다음과 같이 `Document` 객체(`page_content`와 선택적 메타데이터 포함)를 추가합니다:
...
`similarity_search`를 사용하여 의미론적 쿼리를 수행하면 가장 가까운 임베딩된 문서를 반환합니다:
...
임베딩 유사도는 다음을 사용하여 계산할 수 있습니다:

* **코사인 유사도(Cosine similarity)**
* **유클리드 거리(Euclidean distance)**
* **내적(Dot product)**

효율적인 검색은 종종 HNSW(Hierarchical Navigable Small World)와 같은 인덱싱 방법을 사용하지만, 구체적인 내용은 벡터 스토어에 따라 다릅니다.

### 메타데이터 필터링

메타데이터(예: 출처, 날짜)로 필터링하여 검색 결과를 개선할 수 있습니다:
...
<important>
메타데이터 기반 필터링 지원은 구현에 따라 다릅니다.
선택한 벡터 스토어의 문서에서 자세한 내용을 확인하세요.
</important>
...
vector_store = Chroma(
    ...
    persist_directory="./chroma_langchain_db",  # 로컬에 데이터를 저장할 위치, 필요하지 않으면 제거
)
```
</Accordion>
...
| Vectorstore | ID로 삭제 | 필터링 | 벡터로 검색 | 점수와 함께 검색 | 비동기 | 표준 테스트 통과 | 멀티 테넌시 | 문서 추가 시 ID |
|------------|---------|-------|-----------|--------------|--------|---------------|-----------|---------------|
...
<Card title="Azure Cosmos DB Mongo vCore" icon="link" href="/oss/python/integrations/vectorstores/azure_cosmos_db" arrow="true" cta="가이드 보기"/>
