...
[`create_agent`](https://reference.langchain.com/python/langchain/agents/#langchain.agents.create_agent)에 미들웨어를 전달하여 추가합니다:
...
<ParamField body="allowed_decisions" type="list[string]">
  허용되는 결정 목록: `"approve"`, `"edit"` 또는 `"reject"`
</ParamField>
...
<Note>
**중요:** 휴먼 인 더 루프 미들웨어는 중단 간 상태를 유지하기 위해 [체크포인터](/oss/python/langgraph/persistence#checkpoints)가 필요합니다.

전체 예제 및 통합 패턴은 [휴먼 인 더 루프 문서](/oss/python/langchain/human-in-the-loop)를 참조하세요.
</Note>
...
<ParamField body="detector" type="function | regex">
  사용자 정의 탐지기 함수 또는 정규식 패턴. 제공되지 않으면 PII 유형에 대한 내장 탐지기를 사용합니다.
</ParamField>
...
<ParamField body="on_failure" type="string | callable" default="return_message">
  ...
  - `"return_message"` - 오류 세부 정보가 있는 ToolMessage 반환(LLM이 실패를 처리하도록 허용)
  - `"raise"` - 예외 다시 발생(에이전트 실행 중지)
  - 사용자 정의 호출 가능 - 예외를 받아 ToolMessage 콘텐츠에 대한 문자열을 반환하는 함수
</ParamField>
...
1. **데코레이터 기반** - 단일 훅 미들웨어에 빠르고 간단함
2. **클래스 기반** - 여러 훅을 사용하는 복잡한 미들웨어에 더 강력함
...
- [휴먼 인 더 루프](/oss/python/langchain/human-in-the-loop) - 민감한 작업에 대한 사람의 검토 추가
