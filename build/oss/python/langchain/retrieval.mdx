...
<Columns cols={2}>
    ...
    <Card
        ...
        cta="더 알아보기"
    >
        대용량 문서를 개별적으로 검색 가능하고 모델의 컨텍스트 윈도우에 맞는 작은 청크로 분할합니다.
    </Card>
...
system_prompt = """\
웹 페이지에서 정보를 가져와야 할 때 fetch_url을 사용하세요; 관련 스니펫을 인용하세요.
"""

agent = create_agent(
    model="claude-sonnet-4-0",
    tools=[fetch_url], # 검색을 위한 도구 [!code highlight]
...
def fetch_documentation(url: str) -> str:  # [!code highlight]
    """URL에서 문서를 가져오고 변환합니다"""
    if not any(url.startswith(domain) for domain in ALLOWED_DOMAINS):
        return (
            "오류: URL이 허용되지 않습니다. "
            f"다음 중 하나로 시작해야 합니다: {', '.join(ALLOWED_DOMAINS)}"
        )
    response = requests.get(url, timeout=10.0)
    response.raise_for_status()
...
# llms.txt의 콘텐츠를 가져올 것이므로, LLM 요청 없이
# 미리 수행할 수 있습니다.
llms_txt_content = requests.get(LLMS_TXT).text

# 에이전트를 위한 시스템 프롬프트
system_prompt = f"""
당신은 전문 Python 개발자이자 기술 어시스턴트입니다.
