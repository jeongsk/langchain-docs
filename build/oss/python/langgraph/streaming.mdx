...
다음 스트림 모드 중 하나 이상을 리스트로 [`stream()`](https://reference.langchain.com/python/langgraph/graphs/#langgraph.graph.state.CompiledStateGraph.stream) 또는 [`astream()`](https://reference.langchain.com/python/langgraph/graphs/#langgraph.graph.state.CompiledStateGraph.astream) 메서드에 전달합니다:
...
LangGraph 그래프는 스트리밍 출력을 이터레이터로 생성하기 위해 [`.stream()`](https://reference.langchain.com/python/langgraph/pregel/#langgraph.pregel.Pregel.stream) (동기) 및 [`.astream()`](https://reference.langchain.com/python/langgraph/pregel/#langgraph.pregel.Pregel.astream) (비동기) 메서드를 제공합니다.
