...
1. 워크플로우가 재개될 때 이러한 연산이 특정 실행에 대해 반복되지 않고 대신 지속성 레이어에서 결과를 검색하도록, 비결정론적 연산(예: 난수 생성)이나 부작용이 있는 연산(예: 파일 쓰기, API 호출)을 @[task][task] 안에 래핑합니다. 자세한 내용은 [결정론과 일관된 재실행](#determinism-and-consistent-replay)을 참조하세요.
...
피해야 할 함정의 몇 가지 예는 functional API의 [일반적인 함정](/oss/python/langgraph/functional-api#common-pitfalls) 섹션을 참조하세요. 이 섹션은 이러한 문제를 방지하기 위해 **task**를 사용하여 코드를 구조화하는 방법을 보여줍니다. 동일한 원칙이 [StateGraph (Graph API)](https://reference.langchain.com/python/langgraph/graphs/#langgraph.graph.state.StateGraph)에도 적용됩니다.
