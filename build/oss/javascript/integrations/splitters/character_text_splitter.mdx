...
문자 기반 분할은 텍스트 분할의 가장 간단한 방식입니다. 지정된 문자 시퀀스(기본값: `"\n\n"`)를 사용하여 텍스트를 나누며, 청크 길이는 문자 개수로 측정됩니다.

**주요 사항**:
1. **텍스트 분할 방식**: 지정된 문자 구분자를 기준으로 분할합니다.
2. **청크 크기 측정 방식**: 문자 개수로 측정합니다.


다음 중에서 선택할 수 있습니다:
- `.splitText` — 일반 문자열 청크를 반환합니다.
- `.createDocuments` — LangChain [Document](https://v03.api.js.langchain.com/classes/_langchain_core.documents.Document.html) 객체를 반환하며, 하위 작업에서 메타데이터를 보존해야 할 때 유용합니다.
