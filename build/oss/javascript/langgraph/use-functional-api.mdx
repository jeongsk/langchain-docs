...
[**함수형 API**](/oss/javascript/langgraph/functional-api)를 사용하면 기존 코드를 최소한으로 수정하면서도 LangGraph의 핵심 기능인 [영속성](/oss/javascript/langgraph/persistence), [메모리](/oss/javascript/langgraph/add-memory), [human-in-the-loop](/oss/javascript/langgraph/interrupts), [스트리밍](/oss/javascript/langgraph/streaming)을 애플리케이션에 추가할 수 있습니다.

<Tip>
함수형 API에 대한 개념적인 정보는 [함수형 API](/oss/javascript/langgraph/functional-api)를 참조하세요.
</Tip>
...
**함수형 API**와 [**그래프 API**](/oss/javascript/langgraph/graph-api)는 동일한 기본 런타임을 공유하므로 같은 애플리케이션에서 함께 사용할 수 있습니다.
...
자세한 내용은 [**스트리밍 가이드**](/oss/javascript/langgraph/streaming) 섹션을 참조하세요.
...
함수형 API는 `interrupt` 함수와 `Command` 프리미티브를 사용하여 [human-in-the-loop](/oss/javascript/langgraph/interrupts) 워크플로우를 지원합니다.

### 기본 human-in-the-loop 워크플로우

세 개의 [작업](/oss/javascript/langgraph/functional-api#task)을 생성하겠습니다:
...
이제 이러한 작업을 [entrypoint](/oss/javascript/langgraph/functional-api#entrypoint)에서 구성할 수 있습니다:
...
[interrupt()](/oss/javascript/langgraph/interrupts#pause-using-interrupt)는 작업 내에서 호출되어 사람이 이전 작업의 출력을 검토하고 편집할 수 있게 합니다. 이 경우 `step_1`과 같은 이전 작업의 결과는 유지되므로 `interrupt` 이후 다시 실행되지 않습니다.
...
`step_1` 이후 `interrupt`로 일시 정지했습니다. interrupt는 실행을 재개하기 위한 지침을 제공합니다. 재개하려면 `human_feedback` 작업에서 예상하는 데이터를 포함하는 [Command](/oss/javascript/langgraph/interrupts#resuming-interrupts)를 발행합니다.
...
실행 전에 도구 호출을 검토하려면 [`interrupt`](/oss/javascript/langgraph/interrupts#pause-using-interrupt)를 호출하는 `review_tool_call` 함수를 추가합니다. 이 함수가 호출되면 재개 명령을 발행할 때까지 실행이 일시 정지됩니다.
...
이제 [entrypoint](/oss/javascript/langgraph/functional-api#entrypoint)를 업데이트하여 생성된 도구 호출을 검토할 수 있습니다. 도구 호출이 수락되거나 수정되면 이전과 같은 방식으로 실행합니다. 그렇지 않으면 사람이 제공한 `ToolMessage`를 추가하기만 하면 됩니다. 이전 작업의 결과(이 경우 초기 모델 호출)는 유지되므로 `interrupt` 이후 다시 실행되지 않습니다.
...
단기 메모리를 사용하면 동일한 **스레드 ID**의 서로 다른 **호출** 간에 정보를 저장할 수 있습니다. 자세한 내용은 [단기 메모리](/oss/javascript/langgraph/functional-api#short-term-memory)를 참조하세요.
...
[장기 메모리](/oss/javascript/concepts/memory#long-term-memory)를 사용하면 서로 다른 **스레드 ID** 간에 정보를 저장할 수 있습니다. 이는 한 대화에서 특정 사용자에 대한 정보를 학습하고 다른 대화에서 사용하는 데 유용할 수 있습니다.

## 워크플로우

* 함수형 API를 사용하여 워크플로우를 구축하는 방법에 대한 더 많은 예제는 [워크플로우와 에이전트](/oss/javascript/langgraph/workflows-agents) 가이드를 참조하세요.
