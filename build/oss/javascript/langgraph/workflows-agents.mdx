...
LangGraph는 에이전트와 워크플로우를 구축할 때 [지속성](/oss/javascript/langgraph/persistence), [스트리밍](/oss/javascript/langgraph/streaming), 디버깅 및 [배포](/oss/javascript/langgraph/deploy) 지원을 포함한 여러 이점을 제공합니다.

## 설정

워크플로우 또는 에이전트를 구축하려면 구조화된 출력과 도구 호출을 지원하는 [모든 채팅 모델](/oss/javascript/integrations/chat)을 사용할 수 있습니다. 다음 예제에서는 Anthropic을 사용합니다:
...
워크플로우와 에이전트 시스템은 LLM과 이에 추가하는 다양한 증강을 기반으로 합니다. [도구 호출](/oss/javascript/langchain/tools), [구조화된 출력](/oss/javascript/langchain/structured-output), [단기 메모리](/oss/javascript/langchain/short-term-memory)는 LLM을 요구 사항에 맞게 조정하는 몇 가지 옵션입니다.
...
평가자-최적화 워크플로우에서는 하나의 LLM 호출이 응답을 생성하고 다른 하나가 해당 응답을 평가합니다. 평가자 또는 [휴먼-인-더-루프](/oss/javascript/langgraph/interrupts)가 응답을 개선해야 한다고 판단하면 피드백이 제공되고 응답이 다시 생성됩니다. 이 루프는 허용 가능한 응답이 생성될 때까지 계속됩니다.
...
에이전트는 일반적으로 [도구](/oss/javascript/langchain/tools)를 사용하여 작업을 수행하는 LLM으로 구현됩니다. 지속적인 피드백 루프에서 작동하며 문제와 솔루션이 예측 불가능한 상황에서 사용됩니다. 에이전트는 워크플로우보다 더 많은 자율성을 가지며 사용하는 도구와 문제 해결 방법에 대한 결정을 내릴 수 있습니다. 사용 가능한 도구 세트와 에이전트 동작에 대한 가이드라인은 여전히 정의할 수 있습니다.

![agent.png](/oss/images/agent.png)

<Note>
에이전트를 시작하려면 [빠른 시작](/oss/javascript/langchain/quickstart)을 참조하거나 LangChain에서 [에이전트가 작동하는 방식](/oss/javascript/langchain/agents)에 대해 자세히 읽어보세요.
</Note>
