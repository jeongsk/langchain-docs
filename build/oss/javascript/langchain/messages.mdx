...
- <Icon icon="wrench" size={16} /> [Tool message](#tool-message) - [도구 호출](/oss/javascript/langchain/models#tool-calling)의 출력을 나타냅니다
...
@[`HumanMessage`]는 사용자 입력과 상호작용을 나타냅니다. 텍스트, 이미지, 오디오, 파일 및 기타 멀티모달 [콘텐츠](#content)를 포함할 수 있습니다.
...
[`AIMessage`](https://v03.api.js.langchain.com/classes/_langchain_core.messages_ai_message.AIMessage.html)는 모델 호출의 출력을 나타냅니다. 멀티모달 데이터, 도구 호출 및 나중에 액세스할 수 있는 제공자별 메타데이터를 포함할 수 있습니다.
...
제공자는 메시지 타입에 따라 다르게 가중치를 부여하고 컨텍스트화하므로, 때로는 새로운 [`AIMessage`](https://v03.api.js.langchain.com/classes/_langchain_core.messages_ai_message.AIMessage.html) 객체를 생성하여 마치 모델에서 나온 것처럼 메시지 기록에 삽입하는 것이 유용합니다.
...
<Accordion title="속성">
    ...
    <ParamField path="content_blocks" type="ContentBlock.Standard[]">
        메시지의 표준화된 콘텐츠 블록입니다. ([콘텐츠](#content) 참조)
    </ParamField>
...
모델이 [도구 호출](/oss/javascript/langchain/models#tool-calling)을 수행하면 [`AIMessage`](https://v03.api.js.langchain.com/classes/_langchain_core.messages_ai_message.AIMessage.html)에 포함됩니다:
...
추론이나 인용과 같은 다른 구조화된 데이터도 메시지 [콘텐츠](/oss/javascript/langchain/messages#content)에 나타날 수 있습니다.

#### 토큰 사용량

[`AIMessage`](https://v03.api.js.langchain.com/classes/_langchain_core.messages_ai_message.AIMessage.html)는 `usage_metadata` 필드에 토큰 수 및 기타 사용 메타데이터를 보유할 수 있습니다:
...
스트리밍 중에는 전체 메시지로 결합할 수 있는 [`AIMessageChunk`](https://v03.api.js.langchain.com/classes/_langchain_core.messages_ai_message.AIMessageChunk.html) 객체를 받게 됩니다:
...
<Note>
더 알아보기:
- [채팅 모델에서 토큰 스트리밍](/oss/javascript/langchain/models#stream)
- [에이전트에서 토큰 및/또는 단계 스트리밍](/oss/javascript/langchain/streaming)
</Note>
...
[도구 호출](/oss/javascript/langchain/models#tool-calling)을 지원하는 모델의 경우 AI 메시지에 도구 호출이 포함될 수 있습니다. 도구 메시지는 단일 도구 실행 결과를 모델에 다시 전달하는 데 사용됩니다.

[도구](/oss/javascript/langchain/tools)는 @[`ToolMessage`] 객체를 직접 생성할 수 있습니다. 아래에 간단한 예제를 보여드립니다. 자세한 내용은 [도구 가이드](/oss/javascript/langchain/tools)를 참조하세요.
...
<Accordion title="속성">
    ...
    <ParamField path="tool_call_id" type="string" required>
        이 메시지가 응답하는 도구 호출의 ID입니다. ([`AIMessage`](https://v03.api.js.langchain.com/classes/_langchain_core.messages_ai_message.AIMessage.html)의 도구 호출 ID와 일치해야 함)
    </ParamField>
...
<Note>
    ...
    <Accordion title="예제: 검색 메타데이터에 artifact 사용">
        예를 들어, [검색](/oss/javascript/langchain/retrieval) 도구는 모델이 참조할 문서에서 구절을 검색할 수 있습니다. 메시지 `content`에 모델이 참조할 텍스트가 포함되어 있는 경우, `artifact`에는 애플리케이션이 사용할 수 있는 문서 식별자 또는 기타 메타데이터가 포함될 수 있습니다(예: 페이지 렌더링). 아래 예제를 참조하세요:
        ...
        LangChain으로 검색 [에이전트](/oss/javascript/langchain/agents)를 구축하는 엔드투엔드 예제는 [RAG 튜토리얼](/oss/javascript/langchain/rag)을 참조하세요.
    </Accordion>
</Note>
...
메시지 객체는 `content` 속성을 표준 타입 안전 표현으로 지연 파싱하는 `contentBlocks` 속성을 구현합니다. 예를 들어, [ChatAnthropic](/oss/javascript/integrations/chat/anthropic) 또는 [ChatOpenAI](/oss/javascript/integrations/chat/openai)에서 생성된 메시지에는 각 제공자의 형식으로 `thinking` 또는 `reasoning` 블록이 포함되지만 일관된 [`ReasoningContentBlock`](#content-block-reference) 표현으로 지연 파싱될 수 있습니다:
...
[채팅 모델](/oss/javascript/langchain/models)은 멀티모달 데이터를 입력으로 받아들이고 출력으로 생성할 수 있습니다. 아래에서 멀티모달 데이터가 포함된 입력 메시지의 짧은 예제를 보여드립니다:
...
[채팅 모델](/oss/javascript/langchain/models)은 메시지 객체의 시퀀스를 입력으로 받아 [`AIMessage`](https://v03.api.js.langchain.com/classes/_langchain_core.messages_ai_message.AIMessage.html)를 출력으로 반환합니다. 상호작용은 종종 상태 비저장이므로 간단한 대화 루프는 증가하는 메시지 목록으로 모델을 호출하는 것을 포함합니다.
