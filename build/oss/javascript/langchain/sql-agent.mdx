...
LangChain 에이전트는 에이전트 도구 호출에 감독을 추가하기 위한 내장 [휴먼 인 더 루프 미들웨어](/oss/javascript/langchain/human-in-the-loop) 지원 기능을 제공합니다. `sql_db_query` 도구 호출 시 사람의 검토를 위해 일시 중지하도록 에이전트를 구성해보겠습니다:
...
```
<Note>
실행을 일시 중지하고 재개할 수 있도록 에이전트에 [체크포인터](/oss/javascript/langchain/short-term-memory)를 추가했습니다. 이에 대한 자세한 내용과 사용 가능한 미들웨어 구성은 [휴먼 인 더 루프 가이드](/oss/javascript/langchain/human-in-the-loop)를 참조하세요.
</Note>
...
```
이 경우 쿼리를 승인하여 [Command](/oss/javascript/langgraph/use-graph-api#combine-control-flow-and-state-updates-with-command)를 사용하여 실행을 재개할 수 있습니다:
```python
...
자세한 내용은 [휴먼 인 더 루프 가이드](/oss/javascript/langchain/human-in-the-loop)를 참조하세요.
...
더 깊은 사용자 정의를 원한다면 LangGraph 프리미티브를 사용하여 SQL 에이전트를 직접 구현하는 [이 튜토리얼](/oss/javascript/langgraph/sql-agent)을 확인하세요.
