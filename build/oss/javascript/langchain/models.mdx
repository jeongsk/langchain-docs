...
* <Icon icon="hammer" size={16} /> [도구 호출](#tool-calling) - 외부 도구(데이터베이스 쿼리 또는 API 호출 등)를 호출하고 그 결과를 응답에 사용합니다.
* <Icon icon="shapes" size={16} /> [구조화된 출력](#structured-outputs) - 모델의 응답이 정의된 형식을 따르도록 제한됩니다.
* <Icon icon="image" size={16} /> [멀티모달](#multimodal) - 이미지, 오디오, 비디오 등 텍스트 이외의 데이터를 처리하고 반환합니다.
* <Icon icon="brain" size={16} /> [추론](#reasoning) - 모델이 결론에 도달하기 위해 다단계 추론을 수행합니다.

모델은 [에이전트](/oss/javascript/langchain/agents)의 추론 엔진입니다. 어떤 도구를 호출할지, 결과를 어떻게 해석할지, 언제 최종 답변을 제공할지 결정하는 에이전트의 의사결정 프로세스를 주도합니다.
...
1. **에이전트와 함께** - [에이전트](/oss/javascript/langchain/agents#model)를 생성할 때 모델을 동적으로 지정할 수 있습니다.
