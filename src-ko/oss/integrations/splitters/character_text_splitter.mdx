...
- `.create_documents` — LangChain @[Document] 객체를 반환하며, 다운스트림 작업에서 메타데이터를 보존해야 할 때 유용합니다.
:::
:::js
다음 두 가지 방법 중 선택할 수 있습니다:
- `.splitText` — 일반 문자열 청크를 반환합니다.
- `.createDocuments` — LangChain @[Document] 객체를 반환하며, 다운스트림 작업에서 메타데이터를 보존해야 할 때 유용합니다.
:::
