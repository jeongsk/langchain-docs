...
@[RecursiveCharacterTextSplitter]는 특정 프로그래밍 언어로 작성된 [텍스트를 분할](/oss/integrations/splitters/)하는 데 유용한 사전 구성된 구분자 목록을 포함하고 있습니다.

:::python
지원되는 언어는 `langchain_text_splitters.Language` 열거형에 저장되어 있습니다. 포함된 언어는 다음과 같습니다:
...
특정 언어의 구분자 목록을 확인하려면 이 열거형의 값을 다음 메서드에 전달하세요:
```python
RecursiveCharacterTextSplitter.get_separators_for_language
```

특정 언어에 맞춤화된 분할기를 생성하려면 열거형의 값을 다음 메서드에 전달하세요:
```python
...
:::js
지원되는 언어는 `SupportedTextSplitterLanguages` 타입에 포함되어 있습니다. 포함된 언어는 다음과 같습니다:
...
아래에서 다양한 언어에 대한 예제를 확인할 수 있습니다.
...
:::
:::js
다음은 Python 텍스트 분할기를 사용하는 예제입니다:
