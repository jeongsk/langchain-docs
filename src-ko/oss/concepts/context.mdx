...
<Tabs>
    ...
    State는 agent의 **도구**에서도 접근할 수 있으며, 필요에 따라 state를 읽거나 업데이트할 수 있습니다. 자세한 내용은 [tool calling guide](/oss/langchain/tools#short-term-memory)를 참조하세요.
...
<Tip>
**메모리 활성화**
메모리를 활성화하는 방법에 대한 자세한 내용은 [memory guide](/oss/langgraph/add-memory)를 참조하세요. 이 강력한 기능을 사용하면 여러 호출에 걸쳐 agent의 state를 유지할 수 있습니다. 그렇지 않으면 state는 단일 실행에만 범위가 지정됩니다.
</Tip>

<a id="store"></a>
## Dynamic cross-conversation context

**Dynamic cross-conversation context**는 여러 대화 또는 세션에 걸쳐 지속되는 영구적이고 가변적인 데이터를 나타내며, LangGraph store를 통해 관리됩니다. 여기에는 사용자 프로필, 선호도, 과거 상호작용이 포함됩니다. LangGraph store는 여러 실행에 걸친 [장기 메모리](/oss/concepts/memory#long-term-memory) 역할을 합니다. 이를 사용하여 영구적인 사실(예: 사용자 프로필, 선호도, 이전 상호작용)을 읽거나 업데이트할 수 있습니다.
