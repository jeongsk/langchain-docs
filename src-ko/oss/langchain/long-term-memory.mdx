...
LangChain 에이전트는 장기 메모리를 활성화하기 위해 [LangGraph 영속성](/oss/langgraph/persistence#memory-store)을 사용합니다. 이는 고급 주제이며 사용하려면 LangGraph에 대한 지식이 필요합니다.


## 메모리 저장

LangGraph는 장기 메모리를 [스토어](/oss/langgraph/persistence#memory-store)에 JSON 문서로 저장합니다.
...
메모리 스토어에 대한 자세한 내용은 [영속성](/oss/langgraph/persistence#memory-store) 가이드를 참조하세요.
