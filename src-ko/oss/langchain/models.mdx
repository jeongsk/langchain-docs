...
모델은 [에이전트](/oss/langchain/agents)의 추론 엔진입니다. 어떤 도구를 호출할지, 결과를 어떻게 해석할지, 언제 최종 답변을 제공할지 결정하는 에이전트의 의사결정 프로세스를 주도합니다.
...
1. **에이전트와 함께** - [에이전트](/oss/langchain/agents#model)를 생성할 때 모델을 동적으로 지정할 수 있습니다.
