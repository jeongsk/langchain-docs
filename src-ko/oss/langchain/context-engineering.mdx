...
<Tip>
컨텍스트 엔지니어링이 처음이신가요? [개념 개요](/oss/concepts/context)를 시작으로 다양한 유형의 컨텍스트와 사용 시기를 이해하세요.
</Tip>
...
LangChain [미들웨어](/oss/langchain/middleware)는 LangChain을 사용하는 개발자에게 컨텍스트 엔지니어링을 실용적으로 만드는 내부 메커니즘입니다.
...
<Tabs>
  <Tab title="상태">
    ...
    def state_aware_prompt(request: ModelRequest) -> str:
        # request.messages는 request.state["messages"]의 단축키입니다
...
<Tabs>
  <Tab title="상태">
    ...
    def inject_file_context(
        ...
        """사용자가 이 세션에서 업로드한 파일에 대한 컨텍스트를 주입합니다."""
...
상태를 수정하는 **영구적** 업데이트(예: [생명주기 컨텍스트](#summarization)의 요약 예시)의 경우, `before_model` 또는 `after_model`과 같은 생명주기 훅을 사용하여 대화 기록을 영구적으로 업데이트하세요. 자세한 내용은 [미들웨어 문서](/oss/langchain/middleware)를 참조하세요.
</Note>
...
def search_orders(
    ...
    Args:
        user_id: 사용자의 고유 식별자
        status: 주문 상태: 'pending', 'shipped', 또는 'delivered'
        limit: 반환할 최대 결과 수
    """
    # 구현 여기에
    pass
...
더 많은 예시는 [동적으로 도구 선택하기](/oss/langchain/middleware#dynamically-selecting-tools)를 참조하세요.
...
<Tabs>
  ...
  <Tab title="런타임 컨텍스트">
    ...
    def context_based_model(
        ...
        """런타임 컨텍스트에 따라 모델을 선택합니다."""
        # 런타임 컨텍스트에서 읽기: 비용 등급 및 환경
        cost_tier = request.runtime.context.cost_tier
        environment = request.runtime.context.environment

        if environment == "production" and cost_tier == "premium":
            # 프로덕션 프리미엄 사용자는 최고의 모델을 받습니다
            model = premium_model
        elif cost_tier == "budget":
            # 예산 등급은 효율적인 모델을 받습니다
            model = budget_model
        else:
            # 표준 등급
            model = standard_model
...
더 많은 예시는 [동적 모델](/oss/langchain/agents#dynamic-model)을 참조하세요.
...
class CustomerSupportTicket(BaseModel):
    ...
    category: str = Field(
        description="문제 카테고리: 'billing', 'technical', 'account', 또는 'product'"
    )
    priority: str = Field(
        description="긴급도 수준: 'low', 'medium', 'high', 또는 'critical'"
    )
    summary: str = Field(
        description="고객 문제의 한 문장 요약"
    )
    customer_sentiment: str = Field(
        description="고객의 감정적 톤: 'frustrated', 'neutral', 또는 'satisfied'"
    )
```
:::
...
<Tabs>
  ...
  <Tab title="저장소">
    ...
    class VerboseResponse(BaseModel):
        """세부 정보가 포함된 자세한 응답입니다."""
        answer: str = Field(description="상세한 답변")
        sources: list[str] = Field(description="사용된 출처")
  ...
  <Tab title="런타임 컨텍스트">
    ...
    def context_based_output(
        ...
        """런타임 컨텍스트에 따라 출력 형식을 선택합니다."""
        # 런타임 컨텍스트에서 읽기: 사용자 역할과 환경
        user_role = request.runtime.context.user_role
        environment = request.runtime.context.environment

        if user_role == "admin" and environment == "production":
            # 프로덕션의 관리자는 상세한 출력을 받습니다
            request = request.override(response_format=AdminResponse)
        else:
            # 일반 사용자는 간단한 출력을 받습니다
...
도구에서 상태, 저장소 및 런타임 컨텍스트에 액세스하는 포괄적인 예시는 [도구](/oss/langchain/tools)를 참조하세요.

## 생명주기 컨텍스트

핵심 에이전트 단계 **사이**에 발생하는 일을 제어합니다 - 요약, 가드레일 및 로깅과 같은 교차 관심사를 구현하기 위해 데이터 흐름을 가로챕니다.

[모델 컨텍스트](#model-context)와 [도구 컨텍스트](#tool-context)에서 보았듯이, [미들웨어](/oss/langchain/middleware)는 컨텍스트 엔지니어링을 실용적으로 만드는 메커니즘입니다. 미들웨어를 사용하면 에이전트 생명주기의 모든 단계에 연결하여 다음 중 하나를 수행할 수 있습니다:
...
<Note>
내장 미들웨어의 전체 목록, 사용 가능한 훅 및 사용자 정의 미들웨어를 만드는 방법은 [미들웨어 문서](/oss/langchain/middleware)를 참조하세요.
</Note>
...
4. **내장 미들웨어 사용** - [`SummarizationMiddleware`](/oss/langchain/middleware#summarization), [`LLMToolSelectorMiddleware`](/oss/langchain/middleware#llm-tool-selector) 등 활용
5. **컨텍스트 전략 문서화** - 어떤 컨텍스트가 전달되는지, 그 이유를 명확하게 작성
6. **일시적 vs 영구적 이해**: 모델 컨텍스트 변경은 일시적(호출당)이지만, 생명주기 컨텍스트 변경은 상태에 지속됩니다

## 관련 리소스

- [컨텍스트 개념 개요](/oss/concepts/context) - 컨텍스트 유형 및 사용 시기 이해
- [미들웨어](/oss/langchain/middleware) - 완전한 미들웨어 가이드
- [도구](/oss/langchain/tools) - 도구 생성 및 컨텍스트 액세스
- [메모리](/oss/concepts/memory) - 단기 및 장기 메모리 패턴
- [에이전트](/oss/langchain/agents) - 핵심 에이전트 개념
