...
@[`create_agent`]에 middleware를 전달하여 추가할 수 있습니다:
...
전체 예제와 통합 패턴은 [human-in-the-loop 문서](/oss/langchain/human-in-the-loop)를 참조하세요.
</Note>
...
const agent = createAgent({
  ...
  middleware: [
    modelCallLimitMiddleware({
      ...
      exitBehavior: "end", // 또는 "error"로 예외 throw
    }),
  ],
});
```
...
<ParamField body="exitBehavior" type="string" default="end">
  제한에 도달했을 때의 동작. 옵션: `"end"` (정상 종료) 또는 `"error"` (예외 throw)
</ParamField>
:::
...
const agent = createAgent({
  ...
  middleware: [
    llmToolSelectorMiddleware({
      model: "openai:gpt-4o-mini", // 선택에 더 저렴한 모델 사용
      maxTools: 3, // 가장 관련성 높은 3개 도구로 제한
      alwaysInclude: ["search"], // 특정 도구는 항상 포함
    }),
  ],
});
```
...
<ParamField body="model" type="string">
  도구 선택을 위한 모델. 기본값은 에이전트의 주 모델입니다.
</ParamField>
...
Middleware는 에이전트의 구성을 통해 충족되어야 하는 컨텍스트 요구사항을 정의할 수 있습니다:
...
const agent = createAgent({
  model: "openai:gpt-4o",
  tools: allTools, // 모든 사용 가능한 도구를 미리 등록해야 합니다
  // Middleware를 사용하여 주어진 실행에 관련된 더 작은 하위 집합을 선택할 수 있습니다.
  middleware: [toolSelectorMiddleware],
});
```
