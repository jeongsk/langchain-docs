...
title: ChatTongyi
---
Tongyi Qwen은 Alibaba의 Damo Academy에서 개발한 대규모 언어 모델입니다. 자연어 이해 및 의미 분석을 통해 자연어로 입력된 사용자 입력을 기반으로 사용자 의도를 이해할 수 있습니다. 다양한 도메인과 작업에서 사용자에게 서비스와 지원을 제공합니다. 명확하고 상세한 지침을 제공하면 기대에 더 부합하는 결과를 얻을 수 있습니다.
이 노트북에서는 langchain에서 [Tongyi](https://www.aliyun.com/product/dashscope)를 주로 `Chat`으로 사용하는 방법을 소개하며,
langchain의 `langchain/chat_models` 패키지에 해당합니다.
...
ChatTongyi는 도구와 해당 인수를 설명하고, 모델이 호출할 도구와 해당 도구의 입력이 포함된 JSON 객체를 반환하도록 하는 tool calling API를 지원합니다.
...
제공한 초기 텍스트에서 대규모 모델이 콘텐츠를 계속 생성하도록 활성화합니다.
