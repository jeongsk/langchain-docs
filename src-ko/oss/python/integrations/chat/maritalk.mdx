...
MariTalk은 브라질 회사 [Maritaca AI](https://www.maritaca.ai)가 개발한 어시스턴트입니다.
MariTalk은 포르투갈어를 잘 이해하도록 특별히 훈련된 언어 모델을 기반으로 합니다.

이 노트북은 두 가지 예제를 통해 LangChain과 함께 MariTalk을 사용하는 방법을 보여줍니다:

1. MariTalk을 사용하여 작업을 수행하는 방법에 대한 간단한 예제입니다.
2. LLM + RAG: 두 번째 예제는 MariTalk의 토큰 제한에 맞지 않는 긴 문서에서 답을 찾는 질문에 답하는 방법을 보여줍니다. 이를 위해 간단한 검색기(BM25)를 사용하여 먼저 문서에서 가장 관련성이 높은 섹션을 검색한 다음 MariTalk에 제공하여 답변합니다.

## 설치

먼저, 다음 명령을 사용하여 LangChain 라이브러리(및 모든 종속성)를 설치하세요:
...
chat.maritaca.ai("Chaves da API" 섹션)에서 얻을 수 있는 API 키가 필요합니다.

### 예제 1 - 애완동물 이름 제안

언어 모델인 ChatMaritalk을 정의하고 API 키로 구성해 보겠습니다.
...
긴 텍스트 생성을 포함하는 작업의 경우, 광범위한 기사 작성 또는 큰 문서 번역과 같이 완전한 텍스트를 기다리는 대신 텍스트가 생성됨에 따라 부분적으로 응답을 받는 것이 유리할 수 있습니다. 이는 특히 생성된 텍스트가 광범위한 경우 애플리케이션을 더욱 반응적이고 효율적으로 만듭니다. 이러한 요구 사항을 충족하기 위해 두 가지 접근 방식을 제공합니다: 하나는 동기식이고 다른 하나는 비동기식입니다.
...
### 예제 2 - RAG + LLM: UNICAMP 2024 입학시험 문제 답변 시스템
...
첫 번째 단계는 공지의 정보로 데이터베이스를 만드는 것입니다. 이를 위해 COMVEST 웹사이트에서 공지를 다운로드하고 추출된 텍스트를 500자 윈도우로 분할합니다.
...
이제 데이터베이스가 있으므로 검색기가 필요합니다. 이 예제에서는 간단한 BM25를 검색 시스템으로 사용하지만 다른 검색기(예: 임베딩을 통한 검색)로 대체할 수 있습니다.
