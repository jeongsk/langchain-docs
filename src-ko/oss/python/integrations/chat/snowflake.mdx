...
[Snowflake Cortex](https://docs.snowflake.com/en/user-guide/snowflake-cortex/llm-functions)는 Mistral, Reka, Meta, Google과 같은 회사의 연구원들이 학습한 업계 최고의 대형 언어 모델(LLM)과 Snowflake가 개발한 오픈 엔터프라이즈급 모델인 [Snowflake Arctic](https://www.snowflake.com/en/data-cloud/arctic/)에 즉시 접근할 수 있게 해줍니다.

이 예제는 LangChain을 사용하여 Snowflake Cortex와 상호 작용하는 방법을 설명합니다.

### 설치 및 설정

먼저 아래 명령을 사용하여 `snowflake-snowpark-python` 라이브러리를 설치합니다. 그런 다음 Snowflake에 연결하기 위한 자격 증명을 환경 변수로 구성하거나 직접 전달합니다.
...
이제 `invoke` 또는 `stream` 메서드를 사용하여 채팅 모델을 호출할 수 있습니다.
