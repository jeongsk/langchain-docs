...
이 페이지는 VertexAI [채팅 모델](/oss/langchain/models)을 시작하기 위한 빠른 개요를 제공합니다. ChatVertexAI의 모든 기능과 구성에 대한 자세한 문서는 [API 참조](https://python.langchain.com/api_reference/google_vertexai/chat_models/langchain_google_vertexai.chat_models.ChatVertexAI.html)를 참조하세요.
...
Google Cloud VertexAI 통합은 [Google PaLM 통합](/oss/integrations/chat/google_generative_ai/)과 별개입니다. Google은 GCP를 통해 PaLM의 엔터프라이즈 버전을 제공하기로 선택했으며, 이는 그곳에서 제공되는 모델을 지원합니다.
