...
>[EverlyAI](https://everlyai.xyz)를 사용하면 클라우드에서 ML 모델을 규모에 맞게 실행할 수 있습니다. 또한 [여러 LLM 모델](https://everlyai.xyz)에 대한 API 접근을 제공합니다.

이 노트북은 [EverlyAI Hosted Endpoints](https://everlyai.xyz/)를 위한 `langchain.chat_models.ChatEverlyAI` 사용을 시연합니다.

* `EVERLYAI_API_KEY` 환경 변수 설정
* 또는 `everlyai_api_key` 키워드 인수 사용
...
# EverlyAI Hosted Endpoints에서 제공하는 LLAMA 모델을 사용해 보겠습니다
