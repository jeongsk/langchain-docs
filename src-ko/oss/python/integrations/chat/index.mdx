...
[Chat 모델](/oss/langchain/models)은 일련의 [메시지](/oss/langchain/messages)를 입력으로 받아 메시지를 출력으로 반환하는 언어 모델입니다 <Tooltip tip="이름에 'Chat' 접두사가 없거나 'LLM'이 접미사로 포함된 모델은 일반적으로 chat 모델 인터페이스를 따르지 않는 이전 모델을 가리키며, 대신 문자열을 입력으로 받아 문자열을 출력으로 반환하는 인터페이스를 사용합니다.">(기존의 일반 텍스트 LLM과는 다릅니다)</Tooltip>.
...
| 제공업체 | [Tool 호출](/oss/langchain/tools) | [Structured output](/oss/langchain/structured-output/) | JSON mode | Local | [Multimodal](/oss/langchain/messages#multimodal) | 패키지 |
|----------|------------|-------------------|-----------|-------|------------|---------|
...
<Info>
    통합 기능을 기여하고 싶으시다면 [통합 기능 기여하기](/oss/contributing#add-a-new-integration)를 참조하세요.
</Info>
