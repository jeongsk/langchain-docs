...
시작하고 아래에 표시된 **모든** 기능을 사용하려면 도구 호출을 위해 미세 조정된 모델을 사용하는 것이 좋습니다.

NousResearch의 [
Hermes-2-Pro-Llama-3-8B-GGUF](https://huggingface.co/NousResearch/Hermes-2-Pro-Llama-3-8B-GGUF)를 사용합니다.

> Hermes 2 Pro는 OpenHermes 2.5 Dataset의 업데이트되고 정리된 버전과 자체 개발한 새로운 Function Calling 및 JSON Mode 데이터셋으로 구성된 Nous Hermes 2의 업그레이드 버전입니다. 이 새로운 버전의 Hermes는 뛰어난 일반 작업 및 대화 기능을 유지하면서 Function Calling도 탁월합니다.

로컬 모델에 대한 자세한 내용은 가이드를 참조하세요:

- [로컬에서 LLM 실행](https://python.langchain.com/v0.1/docs/guides/development/local_llms/)
- [RAG와 함께 로컬 모델 사용](https://python.langchain.com/v0.1/docs/use_cases/question_answering/local_retrieval_qa/)
...
첫째, OpenAI Function Calling과 거의 동일하게 작동합니다.

OpenAI에는 도구와 해당 인수를 설명하고 모델이 호출할 도구와 해당 도구에 대한 입력을 포함하는 JSON 객체를 반환하도록 하는 [tool calling](https://platform.openai.com/docs/guides/function-calling) (여기서는 "tool calling"과 "function calling"을 동의어로 사용합니다) API가 있습니다. 도구 호출은 도구를 사용하는 체인 및 에이전트를 구축하고 모델에서 더 일반적으로 구조화된 출력을 얻는 데 매우 유용합니다.

`ChatLlamaCpp.bind_tools`를 사용하면 Pydantic 클래스, dict 스키마, LangChain 도구 또는 심지어 함수를 모델에 도구로 쉽게 전달할 수 있습니다. 내부적으로 이들은 다음과 같은 OpenAI 도구 스키마로 변환됩니다:
...
모든 ChatLlamaCpp 기능 및 구성에 대한 자세한 문서는 API 레퍼런스를 참조하세요: [python.langchain.com/api_reference/community/chat_models/langchain_community.chat_models.llamacpp.ChatLlamaCpp.html](https://python.langchain.com/api_reference/community/chat_models/langchain_community.chat_models.llamacpp.ChatLlamaCpp.html)
