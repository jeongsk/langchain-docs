...
[리트리버](/oss/langchain/retrieval#building-blocks)는 비구조화된 쿼리가 주어졌을 때 문서를 반환하는 인터페이스입니다.
벡터 스토어보다 더 일반적인 개념입니다.
리트리버는 문서를 저장할 수 있을 필요는 없으며, 단지 반환(또는 검색)할 수 있기만 하면 됩니다.
리트리버는 벡터 스토어로부터 생성될 수 있지만, [Wikipedia 검색](/oss/integrations/retrievers/wikipedia/)과 [Amazon Kendra](/oss/integrations/retrievers/amazon_kendra_retriever/)를 포함할 만큼 충분히 광범위합니다.
...
모든 [벡터 스토어](/oss/integrations/vectorstores)는 리트리버로 변환될 수 있습니다. 사용 가능한 벡터 스토어에 대해서는 벡터 스토어 [통합 문서](/oss/integrations/vectorstores/)를 참조하세요.
