...
>[Modern Treasury](https://www.moderntreasury.com/)는 복잡한 결제 운영을 간소화합니다. 이는 돈을 이동하는 제품과 프로세스를 지원하는 통합 플랫폼입니다.
>
>- 은행 및 결제 시스템에 연결
>- 실시간으로 거래 및 잔액 추적
>- 규모에 맞게 결제 운영 자동화

이 노트북은 `Modern Treasury REST API`에서 데이터를 로드하여 LangChain에 수집할 수 있는 형식으로 변환하는 방법과 벡터화를 위한 예제 사용법을 다룹니다.
...
Modern Treasury API에는 Modern Treasury 대시보드의 개발자 설정 내에서 찾을 수 있는 조직 ID와 API 키가 필요합니다.
