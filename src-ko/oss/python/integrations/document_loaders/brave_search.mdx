...
>
> - `Brave Search`는 자체 웹 인덱스를 사용합니다. 2022년 5월 기준으로 100억 개 이상의 페이지를 포함하고 있으며 타사에 의존하지 않고 검색 결과의 92%를 제공하는 데 사용되었습니다. 나머지는 Bing API에서 서버 측으로 또는 (옵트인 기준으로) Google에서 클라이언트 측으로 검색됩니다. Brave에 따르면 인덱스는 스팸 및 기타 저품질 콘텐츠를 피하기 위해 "Google 또는 Bing의 인덱스보다 의도적으로 작게" 유지되었으며 "Brave Search는 롱테일 쿼리를 복구하는 데 Google만큼 좋지 않다"는 단점이 있습니다.
>- `Brave Search Premium`: 2023년 4월부터 Brave Search는 광고가 없는 웹사이트이지만 결국 광고를 포함하는 새로운 모델로 전환되며 프리미엄 사용자는 광고 없는 경험을 얻게 됩니다. 기본적으로 사용자로부터 IP 주소를 포함한 사용자 데이터가 수집되지 않습니다. 옵트인 데이터 수집을 위해서는 프리미엄 계정이 필요합니다.
