...
문서 형식 지정을 위한 사용자 정의 프롬프트 템플릿을 지정할 수도 있습니다:
...
`AirbyteLoader`의 강력한 기능 중 하나는 업스트림 소스에서 대용량 문서를 로드하는 능력입니다. 대규모 데이터셋으로 작업할 때 기본 `.load()` 동작은 느리고 메모리 집약적일 수 있습니다. 이를 방지하려면 `.lazy_load()` 메서드를 사용하여 더 메모리 효율적인 방식으로 문서를 로드할 수 있습니다.
...
`.alazy_load()`를 사용하여 비동기 방식으로 문서를 지연 로드할 수도 있습니다:
...
- `source` (str, 필수): 로드할 Airbyte 소스의 이름입니다.
- `stream` (str, 필수): 로드할 스트림의 이름입니다(Airbyte 소스는 여러 스트림을 반환할 수 있음).
- `config` (dict, 필수): Airbyte 소스에 대한 구성입니다.
- `template` (PromptTemplate, 선택): 문서 형식 지정을 위한 사용자 정의 프롬프트 템플릿입니다.
- `include_metadata` (bool, 선택, 기본값 True): 출력 문서에 모든 필드를 메타데이터로 포함할지 여부입니다.
