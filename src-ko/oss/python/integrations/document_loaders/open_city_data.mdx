...
[Socrata](https://dev.socrata.com/foundry/data.sfgov.org/vw6y-z8j6)는 도시 오픈 데이터를 위한 API를 제공합니다.

[SF crime](https://data.sfgov.org/Public-Safety/Police-Department-Incident-Reports-Historical-2003/tmnf-yvry)과 같은 데이터셋의 경우 오른쪽 상단의 `API` 탭을 참조하세요.

여기서 `dataset identifier`를 제공받을 수 있습니다.

데이터셋 식별자를 사용하여 주어진 city_id(`data.sfgov.org`)에 대한 특정 테이블을 가져옵니다 -

예: [SF 311 데이터](https://dev.socrata.com/foundry/data.sfgov.org/vw6y-z8j6)의 경우 `vw6y-z8j6`.

예: [SF Police 데이터](https://dev.socrata.com/foundry/data.sfgov.org/tmnf-yvry)의 경우 `tmnf-yvry`.
