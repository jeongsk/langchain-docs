...
[Docling](https://github.com/DS4SD/docling)은 PDF, DOCX, PPTX, HTML 및 기타 형식을 RAG와 같은 생성형 AI 워크플로우를 위해 준비된 문서 레이아웃, 표 등을 포함한 풍부한 통합 표현으로 파싱합니다.

이 통합은 `DoclingLoader` 문서 로더를 통해 Docling의 기능을 제공합니다.
...
제시된 `DoclingLoader` 컴포넌트를 사용하면 다음을 수행할 수 있습니다:

- LLM 애플리케이션에서 다양한 문서 유형을 쉽고 빠르게 사용하고,
- 고급 문서 네이티브 그라운딩을 위해 Docling의 풍부한 형식을 활용할 수 있습니다.

`DoclingLoader`는 두 가지 다른 내보내기 모드를 지원합니다:

- `ExportType.DOC_CHUNKS` (기본값): 각 입력 문서를 청크로 분할하고
  그 다음 각 개별 청크를 다운스트림에서 별도의 LangChain Document로 캡처하려는 경우, 또는
- `ExportType.MARKDOWN`: 각 입력 문서를 별도의 LangChain Document로 캡처하려는 경우

예제는 `EXPORT_TYPE` 매개변수를 통해 두 모드를 모두 탐색할 수 있도록 합니다. 설정된 값에 따라 예제 파이프라인이 적절하게 설정됩니다.
...
고급 사용을 위해 `DoclingLoader`에는 다음 매개변수가 있습니다:

- `file_path`: 단일 문자열(URL 또는 로컬 파일) 또는 반복 가능한 소스
- `converter` (선택사항): 사용할 특정 Docling 변환기 인스턴스
- `convert_kwargs` (선택사항): 변환 실행을 위한 특정 kwargs
- `export_type` (선택사항): 사용할 내보내기 모드: `ExportType.DOC_CHUNKS` (기본값) 또는 `ExportType.MARKDOWN`
- `md_export_kwargs` (선택사항): Markdown 모드를 위한 특정 Markdown 내보내기 kwargs
- `chunker` (선택사항): 사용할 특정 Docling 청커 인스턴스 (doc-chunk 모드용)
- `meta_extractor` (선택사항): 사용할 특정 메타데이터 추출기
...
> 참고: `"Token indices sequence length is longer than the specified maximum sequence length..."`라는 메시지는 이 경우 무시할 수 있습니다. 자세한 내용은 [여기](https://github.com/DS4SD/docling-core/issues/119#issuecomment-2577418826)를 참조하세요.
...
> - 다음 예제 파이프라인은 HuggingFace의 Inference API를 사용합니다. LLM 할당량을 늘리려면 `HF_TOKEN` 환경 변수를 통해 토큰을 제공할 수 있습니다.
> - 이 파이프라인의 종속성은 아래와 같이 설치할 수 있습니다(`--no-warn-conflicts`는 Colab의 미리 채워진 Python 환경을 위한 것이며, 더 엄격한 사용을 위해 제거할 수 있습니다):
...
소스에는 구절 제목(즉, 섹션), 페이지 및 정확한 경계 상자를 포함한 풍부한 그라운딩 정보가 포함되어 있습니다.

## API 레퍼런스

- [LangChain Docling 통합 GitHub](https://github.com/docling-project/docling-langchain)
