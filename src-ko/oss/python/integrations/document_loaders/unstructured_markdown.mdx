...
"single"과 "elements"의 두 가지 모드 중 하나로 로더를 실행할 수 있습니다. "single" 모드를 사용하면 문서가 하나의 @[`Document`] 객체로 반환됩니다. "elements" 모드를 사용하면 unstructured 라이브러리가 문서를 `Title` 및 `NarrativeText`와 같은 요소로 분할합니다. 모드 다음에 추가적인 `unstructured` kwargs를 전달하여 다른 `unstructured` 설정을 적용할 수 있습니다.
