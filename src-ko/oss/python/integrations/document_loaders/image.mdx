...
[Unstructured](https://unstructured.io/)를 사용하여 `.jpg` 및 `.png`와 같은 다양한 이미지 형식을 처리합니다. Unstructured를 로컬에서 설정하는 방법에 대한 자세한 지침은 [이 가이드](/oss/integrations/providers/unstructured/)를 참조하세요. 여기에는 필요한 시스템 종속성 설정이 포함됩니다.
