...
테이블은 분할되지 않으며 각 테이블은 하나의 langchain @[`Document`] 객체에 해당합니다.
테이블의 경우 @[`Document`] 객체에는 추가 `metadata` 필드 `type="table"`과
...
첨부 파일의 경우 langchain @[`Document`] 객체에는 추가 메타데이터 필드 `type="attachment"`이 있습니다.
