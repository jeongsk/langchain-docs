...
`BoxLoader` 클래스는 Box에서 비정형 콘텐츠를 LangChain의 @[`Document`] 형식으로 가져오는 데 도움이 됩니다. Box 파일 ID가 포함된 `List[str]` 또는 Box 폴더 ID가 포함된 `str`을 사용하여 이 작업을 수행할 수 있습니다.
...
이 예제에서는 [토큰 인증](https://developer.box.com/guides/authentication/tokens/developer-tokens)을 사용합니다. 이는 모든 [인증 방법](https://developer.box.com/guides/authentication/)과 함께 사용할 수 있습니다. 어떤 방법으로든 토큰을 가져오기만 하면 됩니다. `langchain-box`에서 다른 인증 유형을 사용하는 방법에 대해 자세히 알아보려면 [Box 프로바이더](/oss/integrations/providers/box) 문서를 참조하세요.
...
커넥터는 이 필드를 통합이 작동하는 데 필요한 필드 목록에 추가한 다음 결과를 `"metadata" : { "source" : "source, "shared_link" : "shared_link" }`와 같이 @[`Document`] 또는 `Blob`에 반환된 메타데이터에 추가합니다. 해당 파일에서 필드를 사용할 수 없는 경우 `"shared_link" : ""`와 같이 빈 문자열로 반환됩니다.
