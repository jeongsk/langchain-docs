...
이 노트북은 LangSmith의 LLM 실행에서 직접 데이터를 로드하고 해당 데이터로 모델을 파인튜닝하는 방법을 보여줍니다.
프로세스는 간단하며 3단계로 구성됩니다.

1. 학습할 LLM 실행을 선택합니다.
2. LangSmithRunChatLoader를 사용하여 실행을 채팅 세션으로 로드합니다.
3. 모델을 파인튜닝합니다.

그런 다음 LangChain 앱에서 파인튜닝된 모델을 사용할 수 있습니다.

본격적으로 시작하기 전에 전제 조건을 설치하겠습니다.

## 전제 조건

langchain >= 0.0.311이 설치되어 있고 LangSmith API 키로 환경을 구성했는지 확인하세요.
...
첫 번째 단계는 파인튜닝할 실행을 선택하는 것입니다. 일반적인 경우는 긍정적인 사용자 피드백을 받은 트레이스 내에서 LLM 실행을 선택하는 것입니다. 이에 대한 예제는 [LangSmith Cookbook](https://github.com/langchain-ai/langsmith-cookbook/blob/main/exploratory-data-analysis/exporting-llm-runs-and-feedback/llm_run_etl.ipynb)과 [문서](https://docs.smith.langchain.com/tracing/use-cases/export-runs/local)에서 찾을 수 있습니다.

이 튜토리얼을 위해 여기에서 사용할 몇 가지 실행을 생성하겠습니다. 간단한 함수 호출 체인을 파인튜닝해 보겠습니다.
...
#### 채팅 세션이 로드되면 파인튜닝에 적합한 형식으로 변환합니다
...
파인튜닝 후 LangChain 앱의 ChatOpenAI 모델 클래스와 함께 결과 모델 ID를 사용합니다.
...
이제 LangSmith LLM 실행의 데이터를 사용하여 모델을 성공적으로 파인튜닝했습니다!
