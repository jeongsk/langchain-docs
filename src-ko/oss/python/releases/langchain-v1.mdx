...
@[`create_agent`]는 LangChain 1.0에서 에이전트를 구축하는 표준 방법입니다. `langgraph.prebuilt.create_react_agent`보다 간단한 인터페이스를 제공하면서도 [미들웨어](#middleware)를 사용하여 더 큰 커스터마이징 가능성을 제공합니다.
...
내부적으로 @[`create_agent`]는 기본 에이전트 루프를 기반으로 구축됩니다 -- 모델을 호출하고, 실행할 도구를 선택하게 한 후, 더 이상 도구를 호출하지 않으면 종료합니다:
...
미들웨어는 @[`create_agent`]의 핵심 기능입니다. 매우 커스터마이징 가능한 진입점을 제공하여 구축할 수 있는 범위의 상한선을 높입니다.

훌륭한 에이전트는 [컨텍스트 엔지니어링](/oss/langchain/context-engineering)이 필요합니다: 적절한 시기에 모델에 올바른 정보를 제공하는 것입니다. 미들웨어는 구성 가능한 추상화를 통해 동적 프롬프트, 대화 요약, 선택적 도구 접근, 상태 관리 및 가드레일을 제어할 수 있도록 도와줍니다.

#### 사전 구축된 미들웨어

LangChain은 일반적인 패턴을 위한 몇 가지 [사전 구축된 미들웨어](/oss/langchain/middleware#built-in-middleware)를 제공합니다:
...
@[`create_agent`]가 [LangGraph](/oss/langgraph)를 기반으로 구축되었기 때문에, 다음을 통해 장기 실행 및 신뢰할 수 있는 에이전트를 위한 기본 지원을 자동으로 받게 됩니다:
...
@[`create_agent`]는 개선된 구조화된 출력 생성 기능을 제공합니다:
...
새로운 @[`content_blocks`][BaseMessage(content_blocks)] 속성은 프로바이더 전반에서 작동하는 메시지 콘텐츠의 표준 표현을 도입합니다:
...
- **하위 호환성**: 표준 콘텐츠를 [지연 로드](/oss/langchain/messages#standard-content-blocks)할 수 있으므로 관련된 호환성 문제가 없습니다

자세한 내용은 [콘텐츠 블록](/oss/langchain/messages#standard-content-blocks) 가이드를 참조하세요.
...
| @[`langchain.messages`] | 메시지 타입, @[콘텐츠 블록][ContentBlock], @[`trim_messages`] | @[`langchain-core`]에서 재내보내기 |
...
LangChain v1로 코드를 업데이트하는 방법은 [마이그레이션 가이드](/oss/migrate/langchain-v1)를 참조하세요.
...
<CardGroup cols={3}>
    ...
    <Card title="메시지 콘텐츠" icon="message" href="/oss/langchain/messages#message-content" arrow>
        새로운 콘텐츠 블록 API
    </Card>
...
- [버저닝](/oss/versioning) - 버전 번호 이해하기
- [릴리스 정책](/oss/release-policy) - 자세한 릴리스 정책
