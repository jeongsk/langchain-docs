...
Open SWE를 사용해 보고 싶지만 어디서 시작해야 할지 모르겠다면, 다음 예제 작업들을 시도해 보세요.

<Steps>
  <Step title="TypeScript 템플릿 클론하기">
    아래의 모든 예제는 TypeScript 작업을 위한 것입니다. 이를 위해 빈 TypeScript 템플릿 저장소를 클론해야 합니다:

    [bracesproul/typescript-template](https://github.com/bracesproul/typescript-template)

    GitHub UI를 방문하여 `Use this template`을 클릭해 이 템플릿을 기반으로 새 저장소를 생성하세요.

  </Step>

  <Step title="Open SWE에 저장소 접근 권한 부여하기">
    처음 설정할 때 Open SWE에 모든 저장소에 대한 권한을 부여하지 않았다면, 이 저장소에 대한 접근 권한을 부여해야 합니다.

    1. Open SWE의 설정 페이지를 방문하세요: [swe.langchain.com/settings](https://swe.langchain.com/settings)
    2. `Current User` 섹션 내에서 사용자 오른쪽에 있는 `+` 버튼을 클릭하세요. GitHub로 리디렉션되어 Open SWE가 저장소에 접근할 수 있도록 권한을 부여할 수 있습니다.
  ...
  <Step title="예제 작업 중 하나 제출하기">
    템플릿을 클론하고 Open SWE에 접근 권한을 부여한 후에는 예제 작업 중 하나를 제출할 수 있습니다!
  </Step>
</Steps>

다음은 시도해 볼 수 있는 일련의 예제들입니다:
...
<Accordion title="GitHub 이슈 동기화 CLI">
  ```txt
...
<Accordion title="JWT + 리프레시 토큰 인증 레이어">
  ```txt
...
<Accordion title="REST API를 감싸는 GraphQL 래퍼">
  ```txt
...
<Accordion title="서명된 URL을 사용한 S3 이미지 업로드">
  ```txt
