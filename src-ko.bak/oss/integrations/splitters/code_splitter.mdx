...
@[RecursiveCharacterTextSplitter]는 특정 프로그래밍 언어로 작성된 [텍스트를 분할](/oss/integrations/splitters/)하는 데 유용한 구분자 목록을 사전에 포함하고 있습니다.

:::python
지원되는 언어는 `langchain_text_splitters.Language` 열거형에 저장되어 있습니다. 포함되는 언어는 다음과 같습니다:
...
:::js
지원되는 언어는 `SupportedTextSplitterLanguages` 타입에 저장되어 있습니다. 포함되는 언어는 다음과 같습니다:
