...
- **에이전트 커스터마이징**: 사용자 정의 시스템 프롬프트, 도구, 하위 에이전트를 포함한 [커스터마이징 옵션](/oss/deepagents/customization)에 대해 알아보세요.
- **미들웨어 이해**: Deep Agent를 구동하는 [미들웨어 아키텍처](/oss/deepagents/middleware)를 자세히 살펴보세요.
- **장기 메모리 추가**: 대화 간 [영구 메모리](/oss/deepagents/long-term-memory)를 활성화하세요.
