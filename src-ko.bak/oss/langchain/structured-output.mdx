...
LangChain의 @[`create_agent`]는 구조화된 출력을 자동으로 처리합니다. 사용자가 원하는 구조화된 출력 스키마를 설정하면, 모델이 구조화된 데이터를 생성할 때 이를 캡처하고 검증한 후 에이전트 상태의 `'structured_response'` 키에 반환합니다.
...
- 네이티브 구조화된 출력을 지원하는 모델의 경우 `ProviderStrategy` (예: [OpenAI](/oss/integrations/providers/openai), [Grok](/oss/integrations/providers/xai))
...
스키마 타입을 @[`create_agent.response_format`][create_agent(response_format)]에 직접 전달하고 모델이 네이티브 구조화된 출력을 지원하는 경우, LangChain이 자동으로 `ProviderStrategy`를 사용합니다:
