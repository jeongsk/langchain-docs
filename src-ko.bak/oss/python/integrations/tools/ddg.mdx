...
이 가이드는 DuckDuckGo 검색 컴포넌트를 사용하는 방법을 보여드립니다.
...
추가 정보(예: 링크, 출처)를 얻으려면 `DuckDuckGoSearchResults()`를 사용하세요.
...
기본적으로 결과는 원본 검색 결과의 키-값 쌍을 쉼표로 구분한 문자열로 반환됩니다. `output_format="list"`로 설정하여 검색 결과를 리스트로 반환하거나, `output_format="json"`으로 설정하여 JSON 문자열로 반환할 수도 있습니다.
...
`DuckDuckGoSearchResults`에 커스텀 `DuckDuckGoSearchAPIWrapper`를 직접 전달하여 검색 결과를 더욱 세밀하게 제어할 수도 있습니다.
...
- [채팅 모델을 사용하여 도구 호출하는 방법](https://python.langchain.com/docs/how_to/tool_calling/)
