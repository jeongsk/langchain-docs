...
이 JSON 분할기는 청크 크기를 제어하면서 JSON 데이터를 [분할](/oss/integrations/splitters/)합니다. JSON 데이터를 깊이 우선으로 탐색하여 더 작은 JSON 청크를 만듭니다. 중첩된 JSON 객체를 가능한 한 온전하게 유지하려고 시도하지만, 청크를 min_chunk_size와 max_chunk_size 사이로 유지하기 위해 필요하다면 분할합니다.
