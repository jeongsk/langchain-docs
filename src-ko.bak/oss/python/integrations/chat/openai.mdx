...
이 가이드는 OpenAI [채팅 모델](/oss/langchain/models) 시작하기에 대한 간략한 개요를 제공합니다.
...
<Tip>
    **API Reference**
    ...
    모든 기능과 구성 옵션에 대한 자세한 문서는 @[`ChatOpenAI`] API reference를 참조하세요.
</Tip>

<Info>
    **Azure에서 호스팅되는 OpenAI 모델**

    특정 OpenAI 모델은 [Microsoft Azure platform](https://azure.microsoft.com/en-us/products/ai-foundry/models/openai/)을 통해서도 액세스할 수 있습니다. Azure OpenAI 서비스를 사용하려면 [`AzureChatOpenAI`](/oss/integrations/chat/azure_chat_openai/) 통합을 사용하세요.
</Info>
...
@[`ChatOpenAI`] 또는 `AzureChatOpenAI`로 스트리밍할 때 토큰 수를 복구하려면
...
도구 바인딩 및 도구 호출 출력에 대한 자세한 내용은 [tool calling](/oss/langchain/tools) 문서를 참조하세요.
...
@[`ChatOpenAI`]는 [여러 가지 방법으로 구조화된 출력을 반환](/oss/langchain/structured-output/)할 수 있습니다. 간략한 개요는 아래를 참조하세요:
...
자세한 내용은 [여기](/oss/langchain/structured-output/)를 참조하세요.
...
다른 채팅 모델과 마찬가지로 [LangGraph](/oss/langgraph/quickstart)를 사용하거나 수동으로 상태를 관리할 수 있습니다:
...
<Tip>
**[LangGraph](https://langchain-ai.github.io/langgraph/)를 사용하여 인메모리 및 Postgres를 포함한 다양한 백엔드에서 대화 스레드를 관리할 수 있습니다. 시작하려면 [이 튜토리얼](/oss/langgraph/quickstart)을 참조하세요.**
...
OpenAI는 모델의 추론 요약을 반환할 수 있습니다(원시 추론 토큰은 노출하지 않음). 이 요약을 반환하도록 @[`ChatOpenAI`]를 구성하려면 `reasoning` 매개변수를 지정하세요. 이 매개변수가 설정되면 @[`ChatOpenAI`]는 자동으로 Responses API로 라우팅됩니다.
...
OpenAI에는 멀티모달 입력을 지원하는 모델이 있습니다. 이러한 모델에 이미지, PDF 또는 오디오를 전달할 수 있습니다. LangChain에서 이를 수행하는 방법에 대한 자세한 내용은 [multimodal inputs](/oss/langchain/messages#multimodal) 문서를 참조하세요.
...
멀티모달 데이터를 @[`ChatOpenAI`]에 전달하려면 데이터를 포함하는 [콘텐츠 블록](/oss/langchain/messages/)을 생성하고 아래와 같이 메시지에 통합하세요:
...
사용 방법 가이드의 예제는 [여기](/oss/langchain/messages#multimodal)를 참조하세요.
...
자세한 내용은 [여기](/oss/langchain/messages#multimodal#example-openai-file-names)를 참조하세요.

사용 방법 가이드의 예제는 [여기](/oss/langchain/messages#multimodal#documents-pdf)를 참조하세요.
...
사용 방법 가이드의 예제는 [여기](/oss/langchain/messages#multimodal#audio)를 참조하세요.
