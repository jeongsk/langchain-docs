---
title: LangChain용 Redis 캐시
---

이 노트북은 langchain-redis 패키지의 `RedisCache` 및 `RedisSemanticCache` 클래스를 사용하여 LLM 응답에 대한 캐싱을 구현하는 방법을 보여줍니다.
...
이 노트북에서는 langchain-redis 패키지의 `RedisCache`와 `RedisSemanticCache`의 사용법을 시연했습니다. 이러한 캐싱 메커니즘은 중복된 API 호출을 줄이고 지능적인 캐싱을 위해 의미적 유사성을 활용함으로써 LLM 기반 애플리케이션의 성능을 크게 향상시킬 수 있습니다. Redis 기반 구현은 분산 시스템에서 캐싱을 위한 빠르고 확장 가능하며 유연한 솔루션을 제공합니다.
