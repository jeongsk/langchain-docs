...
<Note>
셀프 호스팅 설치 환경이나 EU 리전의 조직인 경우, 아래 요청에서 LangSmith URL을 적절히 업데이트하세요. EU 리전의 경우 `eu.api.smith.langchain.com`을 사용하세요.
</Note>
...
1. OpenTelemetry 지원이 포함된 LangSmith 패키지를 설치합니다:
   ...
   <Info>
   Python SDK 버전 `langsmith>=0.3.18`이 필요합니다. 중요한 OpenTelemetry 수정 사항의 혜택을 받으려면 `langsmith>=0.4.25`를 권장합니다.
   </Info>
...
3. 추적 기능이 포함된 LangChain 애플리케이션을 생성합니다. 예시:
...
4. 애플리케이션이 실행되면 LangSmith 대시보드에서 추적 정보를 확인할 수 있습니다 ([예시](https://smith.langchain.com/public/a762af6c-b67d-4f22-90a0-728df16baeba/r)).

## LangChain이 아닌 애플리케이션 추적하기

LangChain이 아닌 애플리케이션이나 커스텀 계측의 경우, 표준 OpenTelemetry 클라이언트를 사용하여 LangSmith에서 애플리케이션을 추적할 수 있습니다. (**langsmith ≥ 0.4.25**를 권장합니다.)
...
2. 엔드포인트에 대한 환경 변수를 설정하고, 특정 값으로 대체합니다:
   ...
   <Note>
   otel exporter의 설정 방식에 따라, 추적 데이터만 전송하는 경우 엔드포인트에 `/v1/traces`를 추가해야 할 수 있습니다.
   </Note>

   <Note>
   LangSmith를 셀프 호스팅하는 경우, 기본 엔드포인트를 LangSmith API 엔드포인트로 교체하고 `/api/v1`을 추가하세요. 예시: `OTEL_EXPORTER_OTLP_ENDPOINT=https://ai-company.com/api/v1/otel`
   </Note>
...
4. LangSmith 대시보드에서 추적 정보를 확인합니다 ([예시](https://smith.langchain.com/public/4f2890b1-f105-44aa-a6cf-c777dcc27a37/r)).
...
<Info>
LangSmith Python SDK **≥ 0.4.1**에서 사용 가능합니다. OTEL 내보내기 및 하이브리드 팬아웃 안정성을 개선하는 수정 사항을 위해 **≥ 0.4.25**를 권장합니다.
</Info>
...
2. LangChain 컴포넌트를 초기화하기 전에 전역 trace provider를 설정합니다. LangSmith는 이를 감지하고 자체 제공자를 생성하는 대신 이를 사용합니다.
...
<Info>
하이브리드 추적은 버전 **≥ 0.4.1**에서 사용 가능합니다. OTEL 엔드포인트**만**으로 추적 데이터를 전송하려면 다음을 설정하세요:
...
5. LangSmith 대시보드에서 추적 정보를 확인합니다 ([예시](https://smith.langchain.com/public/106f5bed-edca-4357-91a5-80089252c9ed/r)).
...
2. 여러 대상으로 내보내는 구성 파일(예: `otel-collector-config.yaml`)을 생성합니다:
