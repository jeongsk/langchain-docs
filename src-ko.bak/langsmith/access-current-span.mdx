---
title: 추적된 함수 내에서 현재 실행(span)에 접근하기
sidebarTitle: 추적된 함수 내에서 현재 실행(span)에 접근하기
---

추적된 함수 내에서 현재 실행(span)에 접근해야 하는 경우가 있습니다. 이는 현재 실행에서 UUID, 태그 또는 기타 정보를 추출할 때 유용합니다.

Python 또는 TypeScript SDK에서 각각 `get_current_run_tree`/`getCurrentRunTree` 함수를 호출하여 현재 실행에 접근할 수 있습니다.
