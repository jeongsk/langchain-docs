...
이 가이드는 double-texting이 무엇인지에 대한 지식을 전제로 합니다. 이에 대해서는 [double-texting 개념 가이드](/langsmith/double-texting)에서 배울 수 있습니다.

이 가이드는 double-texting을 위한 `enqueue` 옵션을 다룹니다. 이 옵션은 중단 요청을 큐에 추가하고 클라이언트가 수신한 순서대로 실행합니다. 다음은 `enqueue` 옵션을 사용하는 간단한 예제입니다.

## 설정

먼저, JS 및 CURL 모델 출력을 출력하기 위한 간단한 헬퍼 함수를 정의하겠습니다 (Python을 사용하는 경우 이 단계를 건너뛸 수 있습니다):
...
그런 다음, 필요한 패키지를 임포트하고 클라이언트, 어시스턴트, 스레드를 인스턴스화하겠습니다.
...
이제 두 개의 실행을 시작하겠습니다. 두 번째 실행은 "enqueue" 멀티태스크 전략으로 첫 번째 실행을 중단합니다:
